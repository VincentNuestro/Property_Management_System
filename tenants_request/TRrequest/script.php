<script type="text/javascript">
	...
	function CreateTenantRequest(){
		...
		$("#frmuploadvisit .remove").click();
		$(".tentssave").show();
		$(".tentsview").hide();
		$(".widget-body button").show();
		$("#chckboxnotify").prop('checked',false);
		$("#CreateNewTenantRequest select").prop('disabled',false);
		$("#CreateNewTenantRequest input").prop('disabled',false);
		$("#ApplicationDate").prop('disabled',true);
		$("#CreateNewTenantRequest textarea").prop('disabled',false);
...
/*	END Michael Capistrano 09-10-2019*/
    ...
    function  addthisvistor() {
    	$("#btnuploadvisitimage").click();
    }


    function addthisvisitor2(){
    	...
    	var txtvisitimagename = $("#txtvisitimagename").val();
    	if(txttentfname=="" ){
    		$("#txttentfname").focus();
    		...
    		if(txtvisitimagename!=""){
    			var imageups = '<a><i class="fa fa-paperclip"></i><a/> '+txtvisitimagename.substring(0,10)+"...";
    		}else{
    			var imageups = '';
    		}
    		$("#tbltentvistorslist").append('<tr id="trvis'+cnt+'"><td class="visfname">'+cnt+'</td><td >'+txttentlname+'</td><td>'+txttentfname+'</td><td style="text-align:center">'+txttentidpres+'</td><td>'+imageups+'</td><td>'+txttentlogin+'</td><td>'+txttentlogout+'</td><td><a onclick="removetentvis(\'trvis'+cnt+'\')"><i class="ace-icon glyphicon glyphicon-remove" style="color:red"></i></a></td><td style="display:none">'+txtvisitimagename+'</td></tr>');
    		$("#wdgetvisitors input").val("");
    		$("#frmuploadvisit .remove").click();
    	}
    }
    ...
    function addthisitem(){
    	...
    	if(txttentitemname=="" ){
    		...
    		$("#collapseThree textarea").text("");
    		$("#collapseThree textarea").val("");
    		$("#wdgetitems input").val("");
    		$("#wdgetitems textarea").val("");
    	}
    }
    ...
    function openmodaltr(trid){
		...
		$.ajax({
			...
			success:function(data){
				...
				var arr = data.split("|||");
				$("#CreateNewTenantRequest input").val("");
				$("#CreateNewTenantRequest textarea").text("");
				$("#CreateNewTenantRequest textarea").val("");
				$("#CreateNewTenantRequest input").val("");
				$("#tbltentvistorslist").html("");
				$("#tbltentitemslist").html("");
				$("#chckboxnotify").prop('checked',false);
				...
				$("#ApplicationDate").val("");
				 ...
				 setTimeout(function(){
				 	...
				 	$(".tentssave").hide();
				 	$(".tentsview").html('<i class="fa fa-check"></i>&nbsp;Edit');
    ...
    $('#frmuploadvisit').submit(function(){
      event.preventDefault();              
          $.ajax(
          { 
            url: "tenants_request/uploadclass.php",
            type: 'POST',
            ...
            success: function(data)
            {
                $("#txtvisitimagename").val($.trim(data));
                addthisvisitor2();
            }
          });  
    });
    ...
    function Editrequesttenant(){
    	var btn = $(".tentsview").text();
    	if($.trim(btn)=='Edit'){
    		$(".tentsview").show();
	    	$(".tentssave").hide();
			$("#tbltentvistorslist tr td").each(function(){
		        if($(this).hasClass('trreqview')){
		        	$(this).removeClass('trreqview');
		        	$(this).addClass('trreqadd');
		        }
			...
			$("#ApplicationDate").prop('disabled',true);
			$("#CreateNewTenantRequest textarea").prop('disabled',false);
			$(".tentsview").html('<i class="fa fa-check"></i>&nbsp;Update');
			$(".viewimagex").removeAttr('onclick');
			$(".widget-body button").show();
			$(".delevisit").show();
    	}else{
    		 showmodal("confirm", "Are you sure you want to update this request?", "updatetenantrequest", null, "", null, "1");  
    	}
    	

    }

    function updatetenantrequest(){
		var ApplicationDate = $("#ApplicationDate").val();
		var TenantID = $("#TenantID").val();
		var UnitID = $("#UnitID").val();
		var MallID = $("#MallID").val();
...
/*		 $("#tbltentvistorslist tr td").each(function(){
		  ...
		  var arrviw=[];
		   ...
		   $("#tbltentvistorslist tr").each(function(){
		        var currentRow=$(this);
		  ...
		  var arritem=[];
		   // loop over each table row (tr)
		   $("#tbltentitemslist tr").each(function(){
		        var currentRow=$(this);
		    
		        var xcol1_value=currentRow.find("td:eq(1)").text();
		        var xcol2_value=currentRow.find("td:eq(2)").text();
		        var xcol3_value=currentRow.find("td:eq(3)").text();
		        var xcol4_value=currentRow.find("td:eq(4)").text();
	    ...
	    if(count == 0){
		    $.ajax({
		    	type: 'POST',
				url: 'tenants_request/TRrequest/class.php',
		    	data: '&ApplicationDate=' + ApplicationDate + '&TenantID=' + TenantID + '&UnitID=' + UnitID + '&MallID=' + MallID + '&RequestCat='+ RequestCat + '&RequestTag=' + RequestTag + '&Remarks=' + Remarks +'&isApproval=' + isApproval + '&Attendees=' + Attendees + "&visitors=" +  JSON.stringify(arrviw) + '&itemsx=' + JSON.stringify(arritem) + '&chckboxnotify=' + chckboxnotify + '&txttenantrequet_id=' + txttenantrequet_id + '&form=updateTenantRequest', 
		    	success:function(data){
		    		var arr = data.split("|"); 
	                if(arr[0] == "1"){
	                    setTimeout(function() {
    ...
    function editdetaillogs(){
    	$(".editlogx").click(function(){
	    	if($(this).hasClass('trreqadd')){
	    		$("#txteditlogsx").val($(this).html());
	    		$("#txttreditid").val($(this).closest('tr').attr('id'));
	    		if($(this).hasClass('editin')){
	    			$("#txtedittype").val(1);
	    		}else{
	    			$("#txtedittype").val(0);
	    		}
    ...
    function saveedittime(){
    	var txteditlogsx = $("#txteditlogsx").val();
	    var txttreditid = $("#txttreditid").val();
	    var txtedittype = $("#txtedittype").val();
	    if(txtedittype=='1'){
	    	$("#"+txttreditid+" td:eq(5)").text(txteditlogsx);
	    }else{
	    	$("#"+txttreditid+" td:eq(6)").text(txteditlogsx);
	    }
    ...
    function editdetailimage(){
    	$(".editimagex").click(function(){
    		$("#frmuploadvisitedit .remove").click();
	    	if($(this).hasClass('trreqadd')){
	    		$("#txttreditimageid").val($(this).closest('tr').attr('id'));
    ...
    function saveeditimage(){
    	$("#btnuploadvisitimageedit").click();
    }

     $('#frmuploadvisitedit').submit(function(){
      	event.preventDefault();  
      	var  txttreditimageid = $("#txttreditimageid").val();           
        $.ajax({ 
            url: "tenants_request/uploadclass.php",
            type: 'POST',
            data: new FormData(this),
            ...
            {
            	var xdata = $.trim(data);
                $("#"+txttreditimageid+" td:eq(4)").html('<a class="viewimagex"><i class="fa fa-paperclip"></i> '+xdata.substring(0,10)+'...</a>');
                $("#"+txttreditimageid+" td:eq(8)").text(xdata);
                $("#modaleditimage").modal('hide');
            }
        });  
    });
