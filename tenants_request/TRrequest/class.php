<?php  	
	...
	switch ($_POST['form']) {
		...
		$sql = "INSERT INTO tbltrans_tenantsrequest SET TenantID = '". $_POST['TenantID'] ."', RequestID = '". $RequestID ."', ApplicationDate = '". date('Y-m-d', strtotime($_POST['ApplicationDate'])) ."', UnitID = '". $_POST['UnitID'] ."', MallID = '". $_POST['MallID'] ."', RequestCat = '". $_POST['RequestCat'] ."', RequestTag = '". $_POST['RequestTag'] ."', isApproval = '". $getTagStatus[0] ."', APP_STATUS = '".$getStat."', Remarks = '". $_POST['Remarks'] ."',notify = '".$_POST['chckboxnotify']."', xuser = '".$_SESSION['MMS-UserID']."' ";
			$res = mysql_query($sql, $connection);
			if($res == true) {
				...
				if( $_POST['visitors'] !="" ){
					...
					for ($i=1; $i <= count($arr)-1; $i++) { 
						$arr2 = explode("|", $arr[$i]);
						if($arr2[5]!=""){
							$login = date('H:i:s',strtotime($arr2[5]));
						}else{
							$login = "";
						}
						if($arr2[6]!=""){
							$logout = date('H:i:s',strtotime($arr2[6]));
						}else{
							$logout = "";
						}
						$sql1 = "INSERT INTO tbltrans_tenantsrequest_visitor SET RequestID = '".$RequestID."',fname = '".mysql_real_escape_string($arr2[2])."',lname = '".mysql_real_escape_string($arr2[1])."',idpres = '".mysql_real_escape_string($arr2[3])."' ,login = '".$login."' ,logout = '".$logout."',ximage = '".mysql_real_escape_string($arr2[8])."' ";
		...
		case 'displayTenantRequest':
				$ctr = 1;
				$sql = "SELECT a.id, a.ApplicationDate, b.tradename, b.unitname, c.reqCatDesc, d.reqTagDesc, a.isApproval, a.APP_STATUS,a.RequestID FROM tbltrans_tenantsrequest a,tbltrans_tenants b, tblreqcategory c, tblreqtags d WHERE a.TenantID = b.TenantID AND c.reqCatCode = a.requestCat AND d.reqTagCode = a.requestTag AND a.xuser = '".$_SESSION['MMS-UserID']."' ORDER BY a.xdatetime DESC;";
				$res = mysql_query($sql, $connection);
				while($row = mysql_fetch_array($res)){
					$forApp = "";
					if ( $row[7] == 1 ) {
		...
		case 'showexistrequest':
			$sql = "SELECT TenantID,RequestID,ApplicationDate,UnitID,RequestCat,RequestTag,Remarks,notify,UnitID,MallID,APP_STATUS,TenantID FROM tbltrans_tenantsrequest WHERE RequestID = '".$_POST['trid']."' ";
			$result = mysql_query($sql,$connection) or die(mysql_error());
			$row = mysql_fetch_array($result);

			$sqlvis = "SELECT fname,lname,idpres,login,logout,ximage FROM tbltrans_tenantsrequest_visitor WHERE RequestID = '".$_POST['trid']."' ";
			$resvis = mysql_query($sqlvis,$connection) or die(mysql_error());
			$visitors = "";
			$ctr = 1;
			while ($rowvis = mysql_fetch_array($resvis)) {
				if($rowvis['login']=="" || $rowvis['login']=="00:00:00"){
					$login = "";
				}else{
					$login = date('h:i A',strtotime($rowvis['login']));
				}
				if($rowvis['logout']=="" || $rowvis['logout']=="00:00:00"){
					$logout = "";
				}else{
					$logout = date('h:i A',strtotime($rowvis['logout']));
				}
				if($rowvis['ximage']!=""){
					$images = "<a class='viewimagex' onclick='openimagevisitor(\"".$rowvis['ximage']."\")'><i class='fa fa-paperclip'></i> ".substr($rowvis['ximage'], 0, 10)."...</a>";
				}else{
					$images = "";
				}
				$visitors .= "<tr id='trvis".$ctr."'>";
				$visitors .= "<td>".$ctr."</td>";
				$visitors .= "<td class='visfname'>".$rowvis['lname']."</td>";
				$visitors .= "<td>".$rowvis['fname']."</td>";
				$visitors .= "<td>".$rowvis['idpres']."</td>";
				$visitors .= "<td class='trreqview editimagex'>".$images."</td>";
				$visitors .= "<td class='trreqview editlogx editin' style='color:#337ab7'>".$login."</td>";
				$visitors .= "<td class='trreqview editlogx editout' style='color:#337ab7''>".$logout."</td>";
				$visitors .= "<td class='trreqview ' ><a class='delevisit' style='display:none' onclick='removetentvis(\"trvis".$ctr."\")'><i class='ace-icon glyphicon glyphicon-remove' style='color:red'></i></a></td>";
				$visitors .= "<td style='display:none'>".$rowvis['ximage']."</td>";
				$visitors .= "</tr>";
				$ctr++;
			}

			$sqlitems = "SELECT qty,unitx,notes,itemname FROM tbltrans_tenantsrequest_items WHERE RequestID = '".$_POST['trid']."' ";
			$resitems = mysql_query($sqlitems,$connection);
			$items = "";
			$ctr = 1;
			while ($rowitems = mysql_fetch_array($resitems)) {
				$items .= "<tr id='tritems".$ctr."'>";
				$items .= "<td>".$ctr."</td>";
				$items .= "<td>".$rowitems['itemname']."</td>";
				$items .= "<td>".number_format($rowitems['qty'])."</td>";
				$items .= "<td>".$rowitems['unitx']."</td>";
				$items .= "<td>".$rowitems['notes']."</td>";
				$items .= "<td><a class='delevisit' style='display:none' onclick='removetentvis(\"tritems".$ctr."\")'><i class='ace-icon glyphicon glyphicon-remove' style='color:red'></i></a></td></td>";
				$items .= "</tr>";
				$ctr++;
			}
			
			echo $row['TenantID']."|||".$row['RequestID']."|||". date('m/d/Y', strtotime($row['ApplicationDate']))."|||".$row['UnitID']."|||".$row['RequestCat']."|||".$row['RequestTag']."|||".$row['Remarks']."|||".$row['notify']."|||".$visitors."|||".$items."|||".$row['UnitID']."|||".$row['MallID']."|||".$row['TenantID']."|||".$row['APP_STATUS'];
			
		break;
	
		case 'updateTenantRequest':
			$sql = "UPDATE  tbltrans_tenantsrequest SET TenantID = '". $_POST['TenantID'] ."', UnitID = '". $_POST['UnitID'] ."', MallID = '". $_POST['MallID'] ."', RequestCat = '". $_POST['RequestCat'] ."', RequestTag = '". $_POST['RequestTag'] ."',  Remarks = '". $_POST['Remarks'] ."',notify = '".$_POST['chckboxnotify']."' WHERE  RequestID = '".$_POST['txttenantrequet_id']."'";
				$res = mysql_query($sql, $connection) or die(mysql_error($connection));
				if($res == true) {
					// -- count pang check lang to kung may attendee na nakalagay
					$ar_vis = json_decode($_POST['visitors']);
					if( isset($ar_vis) || !empty($ar_vis) ){
						$delvis = mysql_query("DELETE FROM tbltrans_tenantsrequest_visitor WHERE RequestID = '".$_POST['txttenantrequet_id']."' ");
						
						foreach($ar_vis as  $visitors ) {
					      if($visitors->col5!=""){
								$login = date('H:i:s',strtotime($visitors->col5));
							}else{
								$login = "";
							}
							if($visitors->col6!=""){
								$logout = date('H:i:s',strtotime($visitors->col6));
							}else{
								$logout = "";
							}
							$sql1 = "INSERT INTO tbltrans_tenantsrequest_visitor SET RequestID = '".$_POST['txttenantrequet_id']."',lname = '".mysql_real_escape_string($visitors->col1)."',fname = '".mysql_real_escape_string($visitors->col2)."',idpres = '".mysql_real_escape_string($visitors->col3)."' ,login = '".$login."' ,logout = '".$logout."',ximage = '".mysql_real_escape_string($visitors->col8)."' ";
							$res1 = mysql_query( $sql1 , $connection );
						}
					...
					if( $_POST['itemsx'] !="" ){
						$delvis = mysql_query("DELETE FROM tbltrans_tenantsrequest_items WHERE RequestID = '".$_POST['txttenantrequet_id']."' ");
						$arr = explode("####", $_POST['itemsx']);
						for ($i=1; $i <= count($arr)-1; $i++) { 
							$arr2 = explode("|", $arr[$i]);
							$sql1 = "INSERT INTO tbltrans_tenantsrequest_items SET RequestID = '".$_POST['txttenantrequet_id']."',itemname = '".mysql_real_escape_string($arr2[1])."',qty = '".$arr2[2]."',unitx = '".$arr2[3]."' ,notes = '".mysql_real_escape_string($arr2[4])."' ";
							$res1 = mysql_query( $sql1 , $connection );
						}
						
					}
					$ar_items = json_decode($_POST['itemsx']);
					if( isset($ar_items) || !empty($ar_items) ){
						$delvis = mysql_query("DELETE FROM tbltrans_tenantsrequest_items WHERE RequestID = '".$_POST['txttenantrequet_id']."' ");
						foreach($ar_items as  $itemsx ) {
							$sql1 = "INSERT INTO tbltrans_tenantsrequest_items SET RequestID = '".$_POST['txttenantrequet_id']."',itemname = '".mysql_real_escape_string($itemsx->col1)."',qty = '".$itemsx->col2."',unitx = '".$itemsx->col3."' ,notes = '".mysql_real_escape_string($itemsx->col4)."' ";
							$res1 = mysql_query( $sql1 , $connection );
						}
					}

					echo "1|Request successfully update.";   
				/*	$aaaa = json_decode($_POST['visitors']);
  					echo $aaaa[0]->col1."kkkkkkkk";*/
				} else {
