<script type="text/javascript">
	$(function(){
		...
		$(".chkAllSelectedUnits").click(function(){
			if($(this).is(":checked")){
				$(".chkAddChargeSelectedUnits").prop("checked", true);
			}else{
				$(".chkAddChargeSelectedUnits").prop("checked", false);
			}
		})
	})
  ...
  //       })
        ...
        $(".isOccupancyChanged").addClass("hide");
        $("#tblbodyChargesEscalation").html("");
        $("#divbodyChargesEscalationBreakdown").html("");
		$.ajax({
			type: 'POST',
		...
		// if(MonthCount >= 1 || YearCount >= 1 || DayCount >= 1){
			$.ajax({
				...
				success: function(data){
					...
					if(Charges != ""){
        				$(".isOccupancyChanged").removeClass("hide");
					}
					$(".divChargesEscalation").each(function(){
						$("#tbody"+$(this).attr("id").split("div")[1]).html("");
					})
					// $("#tblNDTCharges").html("");
					// $("#tblbodyChargesEscalation").html("");
					// $("#divbodyChargesEscalationBreakdown").html("");
	...
	function fncgetPaymentSchedule(){
		...
		var Charges = "";
		$("#tblNDTCharges tr").each(function(){
			Charges += $(this).attr("id") + "|" + $(this).find("td").eq(7).text() + "|" + $(this).find("td").eq(4).text().replace(/,/g, "") + "@";
		})
		if(SelectedUnits != "" && ( $("#txtNDTMonthcount").val() != 0 || $("#txtNDTYearcount").val() != 0 || $("#txtGlobalDayCount").val() != 0 ) ){
	        ...
	        $("#tbodyEscalation tr").each(function(){
	        	Escalation += $(this).find("td").eq(0).text() + "|" + parseFloat($(this).find(".txtEscalation").val()) + "|" + $(this).find("td").eq(2).text().replace(/,/g, "") + "|" + $(this).find("td").eq(3).text().replace(/,/g, "") + "@";
	        })
	        var ChargesEscaBR = "";
	        var tableID = "";
	        var trLength = 0;
	    	$(".tbodyChargesEscalation").each(function(){
	    		tableID = $(this).attr("id");
	    		if($("#"+$(this).attr("id")+" tr").length == 0){
	    			ChargesEscaBR += "|0|0|0|" + tableID.split("_")[1] + "|" + tableID.split("_")[2] + "@"
	    		}else{
	    			$("#"+$(this).attr("id")+" tr").each(function(){
		    			ChargesEscaBR += $(this).find("td").eq(0).text() + "|" + $(this).find(".txtEscalation").val().replace(/,/g, "") + "|" + $(this).find("td").eq(2).text().replace(/,/g, "") + "|" + $(this).find("td").eq(3).text().replace(/,/g, "") + "|" + tableID.split("_")[1] + "|" + tableID.split("_")[2] + "@";
		    		})
	    		}
	    	})
			$.ajax({
				type: 'POST',
				url: 'global_form/class.php',
				data: 'RentAmount=' + RentAmount + '&SelectedUnits=' + SelectedUnits + '&PaymentType=' + PaymentType + '&DateFrom=' + DateFrom + '&DateTo=' + DateTo + '&Charges=' + Charges + '&EscaRate=' + EscaRate + '&EscaStart=' + EscaStart + '&EscaYearBasis=' + EscaYearBasis + '&VATSetup=' + VATSetup + '&isDaily=' + isDaily + '&Escalation=' + Escalation + '&ChargesEscaBR=' + ChargesEscaBR + '&form=fncgetPaymentSchedule',
				success: function(data){
					if(data == 1){
						setTimeout(function(){
	...
	function ConfshowModalNDTAddCharges(){
		var SelectedUnits = "";
		$(".txtASU").each(function(){
			SelectedUnits += $(this).val() +"|";
		})
		if(SelectedUnits == ""){
			setTimeout(function(){
				showmodal("alert", "Please add alteast 1 unit first.", "", null, "", null, "1");
			}, 500)
		}else{
			$("#mdl_NDTAddCharges").modal("show");
			showModalNDTAddCharges();
		}
	}
				...
				if(data.trim() == ""){
					...
					$("#tblNDTAddCharges tr").each(function(){
						$(this).click(function(){
							...
							$("#txtACUnit").val($(this).find(".ChargeUnit").text());
							$(".chkAllSelectedUnits").prop("checked", false);
							fncAddUnitList();
						})
					})  
				}
	...
	function fncAddUnitList(){
		...
		$.ajax({
			type: 'POST',
			url: 'global_form/class.php',
			data: 'SelectedUnits=' + SelectedUnits + '&form=fncAddUnitList',
			success: function(data){
				$("#ulAddChargeUnitList").html(data);
			}
		})
	}

	function fncAddSelectedCharge(){
		var ChargeRowCount = $("#txtChargeCount").val();
		var DateFrom = $("#txtNDTdateFrom").val();
		var DateTo = $("#txtNDTdateTo").val();
		var ChargeCode = $("#txtACChargeCoe").val();
		var ChargesDesc = $("#txtAcChargeDesc").val();
		var ChargesRate = $("#txtACAmount").val().replace(/,/g, "");
		var ChargeType = $("#txtACUnit").val();
		var SelectedUnits = "";
		$(".chkAddChargeSelectedUnits").each(function(){
			if($(this).is(":checked")){
				SelectedUnits += $(this).val() +"|";
			}
		})
		if(SelectedUnits == ""){
			setTimeout(function(){
				showmodal("alert", "Please select a unit.", "", null, "", null, "1");
			}, 500)
		}else{
			$.ajax({
				type: 'POST',
				url: 'global_form/class.php',
				data: 'ChargeCode=' + ChargeCode + '&ChargesDesc=' + ChargesDesc + '&ChargesRate=' + ChargesRate + '&ChargeType=' + ChargeType + '&SelectedUnits=' + SelectedUnits + '&form=fncAddSelectedCharge',
				success: function(data){
					$("#tblNDTCharges").append(data);
				}, complete: function(){
					// fncgetPaymentSchedule();
				    getTotalMonthlyCharges();
					$("#TR"+ ChargeCode).remove();
					$("#mdlAddSelectedCharge").modal("hide");
					$("#mdl_NDTAddCharges").modal("hide");
				}
			})
			$.ajax({
				type: 'POST',
				url: 'global_form/class.php',
				data: 'ChargeCode=' + ChargeCode + '&ChargesDesc=' + ChargesDesc + '&SelectedUnits=' + SelectedUnits + '&ChargesRate=' + ChargesRate + '&ChargeRowCount=' + ChargeRowCount + '&form=fncAddSelectedChargeEsca',
				success: function(data){
					var arr = data.split("|");
					$("#tblbodyChargesEscalation").append(arr[0]);
					$("#txtChargeCount").val(arr[1]);
				}, complete: function(){
					$("#tblbodyChargesEscalation tr").each(function(){
						var eto = $(this);
						eto.find(".dipindot").click(function(){
							$("#tblbodyChargesEscalation tr").removeClass("selected");
							$("#tblbodyChargesEscalation tr").find(".txtChargeEscalation").css("color", "#393939");
							eto.addClass("selected");
							eto.find(".txtChargeEscalation").css("color", "white");
						})
					})
				}
			})
			$.ajax({
				type: 'POST',
				url: 'global_form/class.php',
				data: 'ChargeCode=' + ChargeCode + '&SelectedUnits=' + SelectedUnits + '&ChargesRate=' + ChargesRate + '&DateFrom=' + DateFrom + '&DateTo=' + DateTo + '&form=fncAddSelectedChargeEscaBreakdown',
				success: function(data){
					$("#divbodyChargesEscalationBreakdown").append(data);
				}
			})
		}
	}
	...
	function fncgetEscalationSched(){
		...
		if(EscaStart >= 1 && YearBasis >= 1 && EscaRate >= 1){
			$.ajax({
				type: 'POST',
				url: 'global_form/class.php',
				data: '&DateFrom=' + DateFrom + '&DateTo=' + DateTo + '&EscaStart=' + EscaStart + '&YearBasis=' + YearBasis + '&EscaRate=' + EscaRate + '&Rent=' + Rent + '&form=fncgetEscalationSched',
				success: function(data){
	...
	function fncGetNewEscalation(Count){
		var Rent = $("#txtNDTMonthlyRent").val().replace(/,/g, "");
		var LastCount = 0;
		$("#tbodyEscalation tr").each(function(){
			if(parseFloat(LastCount) >= parseFloat(Count)){
				var GetPrevRent = $("#tdEscalationAmount"+parseFloat(LastCount - 1)).text().replace(/,/g, "");
				var escaRent = 0;
				if(GetPrevRent == 0 || GetPrevRent == ""){
					escaRent = Rent;
				}else{
					escaRent = GetPrevRent;
				}
				var tdRate = $("#tdEscaRate"+LastCount).val();
				var escaRate = tdRate / 100;
				var escalation = parseFloat(escaRent) * parseFloat(escaRate);
				var totalRent = parseFloat(escaRent) + parseFloat(escalation);
				$("#tdEscalation"+LastCount).text(escalation.toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,"));
				$("#tdEscalationAmount"+LastCount).text(totalRent.toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,"));
			}
			LastCount++;
		})
		// fncgetPaymentSchedule();
	...
	function fncViewChargeEscaBreakdown(ChargeCode, UnitCode, IDCount, isView){
		if(isView == "1"){
			$(".divChargesEscalation").addClass("hide");
			$("#div-"+ChargeCode+"-"+UnitCode).removeClass("hide");
		}else{
			var DateFrom = $("#txtNDTdateFrom").val();
			var DateTo = $("#txtNDTdateTo").val();
			var EscaStart = $("#txtChargeEscaYearStart"+IDCount).val();
			var YearBasis = $("#txtChargeEscaYearBasis"+IDCount).val();
			var EscaRate = $("#txtChargeEscaRate"+IDCount).val();
			var ChargeRate = $("#txtChargeEscaAMount"+IDCount).val();
			if(EscaStart >= 1 && YearBasis >= 1 && EscaRate >= 1){
				$.ajax({
					type: 'POST',
					url: 'global_form/class.php',
					data: '&ChargeCode=' + ChargeCode + '&UnitCode=' + UnitCode + '&DateFrom=' + DateFrom + '&DateTo=' + DateTo + '&EscaStart=' + EscaStart + '&YearBasis=' + YearBasis + '&EscaRate=' + EscaRate + '&ChargeRate=' + ChargeRate + '&form=fncViewChargeEscaBreakdown',
					success: function(data){
						$("#tbody_"+ChargeCode+"_"+UnitCode).html(data);
					}, complete: function(){
				...
				$("#tbody_"+ChargeCode+"_"+UnitCode).html("");
				// fncgetPaymentSchedule();
			}
		}
	}

	function fncGetNewEscalationBreakdown(ChargeCode, UnitCode, Count, Rent){
		var LastCount = 0;
		$("#tbody_"+ChargeCode+"_"+UnitCode+" tr").each(function(){
			if(parseFloat(LastCount) >= parseFloat(Count)){
				var GetPrevRent = $("#tdEscalationAmount-"+ChargeCode+"-"+UnitCode+"-"+parseFloat(LastCount - 1)).text().replace(/,/g, "");
				var escaRent = 0;
				if(GetPrevRent == 0 || GetPrevRent == ""){
					escaRent = Rent;
				}else{
				...
				var tdRate = $("#tdEscaRate-"+ChargeCode+"-"+UnitCode+"-"+LastCount).val();
				var escaRate = tdRate / 100;
				var escalation = parseFloat(escaRent) * parseFloat(escaRate);
				var totalRent = parseFloat(escaRent) + parseFloat(escalation);
				$("#tdEscalation-"+ChargeCode+"-"+UnitCode+"-"+LastCount).text(escalation.toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,"));
				$("#tdEscalationAmount-"+ChargeCode+"-"+UnitCode+"-"+LastCount).text(totalRent.toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,"));
			}
			LastCount++;
		})
		// fncgetPaymentSchedule();
	...
	function fncSaveGlobalForm2(btnAction){
        ...
        var Charges = "";
		$("#tblNDTCharges tr").each(function(){
			Charges += $(this).attr("id") + "|" + $(this).find(".getthisvalue").text().replace(/,/g, "") + "|" + $(this).find("td").eq(7).text() + "@";
		})
        ...
        $.ajax({
            ...
            success: function(data){
        		...
        		<?php if($_GET['url'] != 'inquiry'){ ?>
	            	...
	            	fncLoadChargesEscalation(InquiryID, isProposal);
	            	fncLoadChargesEscalationBR(InquiryID, isProposal);
    ...
    function fncLoadChargesEscalation(InquiryID, isProposal){
    	$.ajax({
    		type: 'POST',
    		url: 'global_form/class.php',
    		data: 'InquiryID=' + InquiryID + '&isProposal=' + isProposal + '&form=fncLoadChargesEscalation',
    		success: function(data){
    			var arr = data.split("|");
    			$("#tblbodyChargesEscalation").html(arr[0]);
    			$("#txtChargeCount").val(arr[1]);
    		}, complete: function(){
    			$("#tblbodyChargesEscalation tr").each(function(){
					var eto = $(this);
    ...
    function fncLoadChargesEscalationBR(InquiryID, isProposal){
    	$.ajax({
    		type: 'POST',
    		url: 'global_form/class.php',
    		data: 'InquiryID=' + InquiryID + '&isProposal=' + isProposal + '&form=fncLoadChargesEscalationBR',
    		success: function(data){
    			$("#divbodyChargesEscalationBreakdown").html(data);
    		}
    	})
    }
	...
	function fncCancelApplication(InquiryID){
		$("#mdlCancelRemark").modal("show");
		$("#btnCancelApplication").attr("onclick", "fncCancelApplication2(\""+ InquiryID +"\")");
	...
	function fncCancelApplication2(InquiryID){
		$("#btnCancelApplication").button('loading');
		$("#txtCancelRemarks").prop("disabled", true);
		var Remarks = $("#txtCancelRemarks").val();
		$.ajax({
			type: 'POST',
			url: 'global_form/class.php',
			data: 'InquiryID=' + InquiryID + '&Remarks=' + Remarks + '&form=fncCancelApplication2',
			success: function(data){
				$("#btnCancelApplication").button('reset');
				$("#txtCancelRemarks").prop("disabled", false);
				if(data == 1){
					setTimeout(function(){
						showmodal("alert", "Application successfully cancelled.", "tblListofApplication", null, "", null, "0");
					}, 500)
					$("#mdlCancelRemark").modal("hide");
				}else{
					setTimeout(function(){
	...
	function getTotalMonthlyCharges(){
		var totalAmount = 0;
		$("#tblNDTCharges tr").each(function(){
			totalAmount += parseFloat($(this).find("td").eq(4).text().replace(/,/g, ""));
		})
    ...
    function fncTestlngto(){
    	var Escalation = "";
    	$("#tblbodyChargesEscalation tr").each(function(){
        	Escalation += $(this).find(".txtChargeEscalation1").val() + "|" + $(this).find(".txtChargeEscalation2").val() + "|" + $(this).find(".txtChargeEscalation3").val() + "|" + $(this).find("td").eq(6).text() + "|" + $(this).find("td").eq(7).text() + "@";
        })
        alert(Escalation)
    }
</script>