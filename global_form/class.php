<?php
	...
	switch ($_POST['form']) {
		...
		case 'fncgetPaymentSchedule':
			...
			$mgaUnits = "";
			$arrUnit = explode("|", $_POST['SelectedUnits']);
			for ($arrUnitCount = 0; $arrUnitCount <= count($arrUnit)-2; $arrUnitCount++) { 
				$mgaUnits .= "'" . $arrUnit[$arrUnitCount] . "'" . ",";
			}
            $TotalArea = 0;
            $UnitCount = 0;
            $UnitInfo = "";
			$resUnitArea = mysql_query("SELECT area, sqm_width, sqm_height, UnitID FROM tblref_unit WHERE unitid IN (". substr(trim($mgaUnits), 0, -1) .");", $connection);
            $UnitCount = mysql_num_rows($resUnitArea);
            if($UnitCount == 1){
                $rowUnit = mysql_fetch_array($resUnitArea);
                ...
                while($rowUnit = mysql_fetch_array($resUnitArea)){
                    $UnitName = mysql_fetch_array(mysql_query("SELECT unitname FROM tblref_unit WHERE unitid = '". $rowUnit['UnitID'] ."';", $connection));
			...
			if($_POST['isDaily'] == 'daily'){
				...
				for ($b = 0; $b <= $NumberofDays; $b++) { 
					...
					echo "<tr id='". date('m/d/Y', strtotime($StartDate)) ."'>
							...
							<td style='text-align: right;' class='lblRentalCharges'>". number_format($TotalRent, 2, '.', ',') ."</td>
						</tr>";
				...
				if($chkBillPeriod == 0){
					...
					while($row = mysql_fetch_array($res)){
						...
						for ($a = 0; $a <= COUNT($Escalation2) -2; $a++) { 
							$arr = explode("|", $Escalation2[$a]);
							if(date('F Y', strtotime($arr[0])) == date("F Y", strtotime($row['EndDate']))){
								$FloatRent = floatval($arr[3]);
								$Escalation = floatval($arr[2]);
								$EscalationRate = $arr[1] ."%";
							}
						}

						$RentAmount = $FloatRent;
						...
						echo "<tr id='". date('m/d/Y', strtotime($row['DueDate'])) ."'>
								<td>". date('m/d/Y', strtotime($row['DueDate'])) ."</td>
								<td class='batayan'>". $UnitInfo ."</td>
						...
						// $arrCharge = explode("@", $_POST['Charges']);
						// for ($b = 0; $b <= COUNT($arrCharge)-2; $b++) { 
						// 	$arrCharge2 = explode("|", $arrCharge[$b]);
			   //          	$ChargeEscalation = explode("@", $_POST['ChargesEscaBR']);
						// 	for ($c = 0; $c <= COUNT($ChargeEscalation) -2; $c++) { 
						// 		$arrChargeEscalation = explode("|", $ChargeEscalation[$c]);
						// 		if( date('F Y', strtotime($arrChargeEscalation[0])) == date("F Y", strtotime($row['EndDate'])) && $arrChargeEscalation[4] == $arrCharge2[0] && $arrChargeEscalation[5] == $arrCharge2[1]){
						// 			$FloatChargeAmount = floatval($arrChargeEscalation[3]);
						// 			$ChargeEscalation = floatval($arrChargeEscalation[2]);
						// 			$ChargeEscalationRate = $arrChargeEscalation[1] ."%";
						// 		}
						// 	}

						// 	$rowCharges = mysql_fetch_array(mysql_query("SELECT CHARGE_DESC FROM tblref_refcharges WHERE CHARGE_ID = '". $arrCharge2[0] ."';", $connection));

						// 	if($_POST['VATSetup'] == 0){
						// 		if($isVatable == "yes"){
			   //                      if($isInclusive == "inc"){ //VAT IS INCLUSIVE
			   //                          $VATAmount = ( floatval($FloatChargeAmount) / 1.12 ) * $VATPercent;
			   //                          $RentLessVAT = floatval($FloatChargeAmount) - $VATAmount;
			   //                          $Rent = $RentLessVAT;
						// 				$VAT = $VATAmount;
			   //                          $TotalRent = $VAT + $Rent;
			   //                      }else{ //VAT IS EXCLUSIVE
			   //                          $VATAmount = floatval($FloatChargeAmount) * $VATPercent;
			   //                          $RentPlusVAT = floatval($FloatChargeAmount);
			   //                          $Rent = floatval($FloatChargeAmount);
			   //                          $VAT = $VATAmount;
			   //                          $TotalRent = floatval($FloatChargeAmount) + $VATAmount;
			   //                      }
			   //                  }else{
			   //                      $Rent = $FloatChargeAmount;
			   //                      $VAT = "0.00";
			   //                      $TotalRent = floatval($FloatChargeAmount);
			   //                  }
			   //              }else{
			   //              	$Rent = $FloatChargeAmount;
		    //                     $VAT = "0.00";
		    //                     $TotalRent = floatval($FloatChargeAmount);
			   //              }
						...
						// 			<td>". $rowCharges['CHARGE_DESC'] ."</td>
						// 			<td style='text-align: right;'>". number_format($Rent, 2, '.', ',') ."</td>
						// 			<td style='text-align: right;'>". $ChargeEscalationRate ."</td>
				 	// 				<td style='text-align: right;'>". number_format($ChargeEscalation, 2, '.', ',') ."</td>
						// 			<td style='text-align: right;'>". number_format(0, 2, '.', ',') ."</td>
						// 			<td style='text-align: right;'>". number_format($VATAmount, 2, '.', ',') ."</td>
						// 			<td style='text-align: right;' class='lblRentalCharges'>". number_format($TotalRent, 2, '.', ',') ."</td>
						// 		</tr>";
		...
		case 'showModalNDTAddCharges':
			...
			$res = mysql_query("SELECT CHARGE_ID, CHARGE_DESC, RATE_TYPE, RATE, OTHER_REASON, CHARGE_TYPE FROM tblref_refcharges WHERE CHARGE_DESC LIKE '%". $_POST['key'] ."%' ORDER BY CHARGE_DESC ASC ", $connection);
			while($row = mysql_fetch_array($res)){
				...
				echo "	<tr id='TR". $row['CHARGE_ID'] ."'>
							...
							<td class='hide ChargeUnit'>". $RateType ."</td>
						</tr>";
			}
		break;
		...
		case 'fncAddUnitList':
			...
			for($i=0; $i <= COUNT($arr)-2; $i++){
				...
				echo 	"<li>
	                        <label>
	                            <input type='checkbox' class='ace chkAddChargeSelectedUnits' value='". $arr[$i] ."'>
	                            <span class='lbl middle padding-4'> ". $UnitName['unitname'] ." </span>
	                        </label>
	                    </li>";
			}
		break;

		case 'fncAddSelectedCharge':
			$arr = explode("|", $_POST['SelectedUnits']);
			for($i = 0; $i <= COUNT($arr)-2; $i++){
				$UnitInfo = mysql_fetch_array(mysql_query("SELECT unitname, area, sqm_height, sqm_width FROM tblref_unit WHERE unitID = '". $arr[$i] ."';", $connection));
                if(SysLeaseSetup('floorandunitmeasurement') == "Area"){
                	$isArea = floatval($UnitInfo['area']);
                }else{
                	$isArea = floatval($UnitInfo['sqm_height'] * $UnitInfo['sqm_width']);
                }

                $ChargeInfo = mysql_fetch_array(mysql_query("SELECT RATE_TYPE FROM tblref_refcharges WHERE CHARGE_ID = '". $_POST['ChargeCode'] ."';", $connection));

                if($ChargeInfo['RATE_TYPE'] == "Persqm"){
            		$ChargeAmount = floatval($_POST['ChargesRate']) * floatval($isArea);
            	}else{
            		$ChargeAmount = floatval($_POST['ChargesRate']);
            	}

				echo 	"<tr id='". $_POST['ChargeCode'] ."' class='Esca-". $_POST['ChargeCode'] ."-". $arr[$i] ."'>
							<td>". $UnitInfo['unitname'] ."</td>
							<td>". number_format($isArea, "2", ".", ",") ." SQM</td>
							<td>". $_POST['ChargesDesc'] ."</td>
							<td>". $_POST['ChargesRate'] ." ". $_POST['ChargeType'] ."</td>
							<td style='text-align: right;'>". number_format($ChargeAmount, "2", ".", ",") ."</td>
							<td class='hide getthisvalue'>". $_POST['ChargesRate'] ."</td>
							<td style='text-align: center;z-index: 0;'><button class='btn btn-xs btn-danger btn-round' onclick='$(\".Esca-". $_POST['ChargeCode'] ."-". $arr[$i] ."\").remove(); fncgetPaymentSchedule(); getTotalMonthlyCharges();'><i class='fa fa-trash-o'></i></button></td>
							<td class='hide'>". $arr[$i] ."</td>
						</tr>";
			}
		break;

		case 'fncAddSelectedChargeEsca':
			$Count = $_POST['ChargeRowCount'];
			$arr = explode("|", $_POST['SelectedUnits']);
			for($i = 0; $i <= COUNT($arr)-2; $i++){
				$UnitInfo = mysql_fetch_array(mysql_query("SELECT unitname, area, sqm_height, sqm_width FROM tblref_unit WHERE unitID = '". $arr[$i] ."';", $connection));
				...
				echo 	"<tr class='Esca-". $_POST['ChargeCode'] ."-". $arr[$i] ."' onclick='fncViewChargeEscaBreakdown(\"". $_POST['ChargeCode'] ."\", \"". $arr[$i] ."\", \"". $i ."\", \"1\")'>
							<td class='dipindot' style='vertical-align: middle;'>". $UnitInfo['unitname'] ."</td>
							<td class='dipindot' style='vertical-align: middle;'>". $_POST['ChargesDesc'] ."</td>
							<td class='dipindot'><input type='text' class='form-control txtChargeEscalation txtChargeEscalation1' style='background-color: transparent; border: 0px; text-align: center; color: #393939;' onkeypress='return isNumberKey(event);' onchange='fncViewChargeEscaBreakdown(\"". $_POST['ChargeCode'] ."\", \"". $arr[$i] ."\", \"". $Count ."\", \"0\");' onkeyup='fncViewChargeEscaBreakdown(\"". $_POST['ChargeCode'] ."\", \"". $arr[$i] ."\", \"". $Count ."\", \"0\");' value='0' id='txtChargeEscaYearStart". $Count ."'></td>
							<td class='dipindot'><input type='text' class='form-control txtChargeEscalation txtChargeEscalation2' style='background-color: transparent; border: 0px; text-align: center; color: #393939;' onkeypress='return isNumberKey(event);' onchange='fncViewChargeEscaBreakdown(\"". $_POST['ChargeCode'] ."\", \"". $arr[$i] ."\", \"". $Count ."\", \"0\");' onkeyup='fncViewChargeEscaBreakdown(\"". $_POST['ChargeCode'] ."\", \"". $arr[$i] ."\", \"". $Count ."\", \"0\");' value='0' id='txtChargeEscaYearBasis". $Count ."'></td>
							<td class='dipindot'>
								<span class='input-icon input-icon-right'>
		                            <input type='text' class='form-control txtChargeEscalation txtChargeEscalation3' style='background-color: transparent; border: 0px; text-align: center; color: #393939;' onkeypress='return isNumberKey(event);' onchange='fncViewChargeEscaBreakdown(\"". $_POST['ChargeCode'] ."\", \"". $arr[$i] ."\", \"". $Count ."\", \"0\");' onkeyup='fncViewChargeEscaBreakdown(\"". $_POST['ChargeCode'] ."\", \"". $arr[$i] ."\", \"". $Count ."\", \"0\");' value='0' id='txtChargeEscaRate". $Count ."'>
		                            <i class='ace-icon fa fa-percent' style='z-index: 0;'></i>
		                        </span>
							</td>
							<td class='hide'><input type='hidden' value='". $ChargeAmount ."' id='txtChargeEscaAMount". $Count ."'></td>
							<td class='hide'>". $_POST['ChargeCode'] ."</td>
							<td class='hide'>". $arr[$i] ."</td>
						</tr>";
					$Count++;
			}
		...
		case 'fncAddSelectedChargeEscaBreakdown':
			...
			$arr = explode("|", $_POST['SelectedUnits']);
			for($a = 0; $a <= COUNT($arr)-2; $a++){
				echo 	"<div style='height: 40vh;' class='divChargesEscalation hide Esca-". $_POST['ChargeCode'] ."-". $arr[$a] ."'  id='div-". $_POST['ChargeCode'] ."-". $arr[$a] ."'>
							<table class='table table-bordered table-striped fixTable'>
						        <thead>
						            <tr>
						                <th style='width: 25%;vertical-align: middle;'>Year</th>
						                <th style='width: 25%;z-index: 1;white-space: nowrap;vertical-align: middle;'>Escalation % Rate</th>
						                <th style='width: 25%;text-align: right;white-space: nowrap;'>Escalation Amount</th>
						                <th style='width: 25%;text-align: right;white-space: nowrap;'>Charge Amount</th>
						            </tr>
						        </thead>
						        <tbody id='tbody_". $_POST['ChargeCode'] ."_". $arr[$a] ."' class='tbodyChargesEscalation'></tbody>
			    			</table>
			    		</div>";
            }
		break;

		case 'fncViewChargeEscaBreakdown':
			$StartYear = date('Y', strtotime($_POST['DateFrom']));
			$EndYear = date('Y', strtotime($_POST['DateTo']));
			...
			if($inYear >= 1){
				...
				for ($i = $YearStart; $i <= $EndYear; $i += $_POST['YearBasis']) { 
					$Escalation = ($_POST['ChargeRate'] + $ComputedEscalation) * floatval($_POST['EscaRate'] / 100);
					...
					echo 	"<tr>
								...
								<td style='text-align: center; vertical-align: middle;z-index: 0;'>
									<span class='input-icon input-icon-right'>
			                            <input type='text' class='form-control txtEscalation' style='background: transparent !important; border: 0px; text-align: center; color: #393939;' onkeypress='return isNumberKey(event);' onchange='fncGetNewEscalationBreakdown(\"". $_POST['ChargeCode'] ."\", \"". $_POST['UnitCode'] ."\", \"". $Count ."\", \"". $_POST['ChargeRate'] ."\");' onkeyup='fncGetNewEscalationBreakdown(\"". $_POST['ChargeCode'] ."\", \"". $_POST['UnitCode'] ."\", \"". $Count ."\", \"". $_POST['ChargeRate'] ."\");' value='". $_POST['EscaRate'] ."' id='tdEscaRate-". $_POST['ChargeCode'] ."-". $_POST['UnitCode'] ."-". $Count ."'>
			                            <i class='ace-icon fa fa-percent' style='z-index: 0;'></i>
			                        </span>
								</td>
								<td style='text-align: right;' id='tdEscalation-". $_POST['ChargeCode'] ."-". $_POST['UnitCode'] ."-". $Count ."'>". number_format($Escalation, 2, '.', ',') ."</td>
								<td style='text-align: right;' id='tdEscalationAmount-". $_POST['ChargeCode'] ."-". $_POST['UnitCode'] ."-". $Count ."'>". number_format($_POST['ChargeRate'] + $ComputedEscalation, 2, '.', ',') ."</td>
								<td class='hide'>". $_POST['ChargeCode'] ."</td>
								<td class='hide'>". $_POST['UnitCode'] ."</td>
							</tr>";
					$Count++;
				}
			}
			...
			if($_POST['InquiryID']== ""){
				...
				if($isDirect == 0){
					...
					if($_POST['MMS_Module'] == 'tenants'){
						...
						for ($i = 0; $i <= COUNT($Escalation2) -2; $i++) { 
							...
							$resInsertEscalation = mysql_query("INSERT INTO tbltrans_escalation SET InquiryID = '". $_POST['InquiryID'] ."', ProposalNum = '". floatval($LastProposalNum[0] + 1) ."', EscaYear = '". $arr[0] ."', EscaRate = '". $arr[1] ."', AccuEscaRate = '". $arr[2] ."', EscaAmount = '". $arr[3] ."', EscaTotal = '". $arr[4] ."';", $connection);
						}

						$ChargeEscalation = explode("@", $_POST['ChargseEsca']);
						for ($i = 0; $i <= COUNT($ChargeEscalation) -2; $i++) { 
							$arr = explode("|", $ChargeEscalation[$i]);
							$resInsertEscalation = mysql_query("INSERT INTO tbltrans_chargeesca SET InquiryID = '". $_POST['InquiryID'] ."', ProposalNum = '". floatval($LastProposalNum[0] + 1) ."', YearStart = '". $arr[0] ."', YearBasis = '". $arr[1] ."', EscaRate = '". $arr[2] ."', ChargeCode = '". $arr[3] ."', UnitID = '". $arr[4] ."';", $connection);
						}

						$ChargeEscalationBR = explode("@", $_POST['ChargesEscaBR']);
						for ($i = 0; $i <= COUNT($ChargeEscalationBR) -2; $i++) { 
							$arr = explode("|", $ChargeEscalationBR[$i]);
							$resInsertEscalation = mysql_query("INSERT INTO tbltrans_chargeesca_br SET InquiryID = '". $_POST['InquiryID'] ."', ProposalNum = '". floatval($LastProposalNum[0] + 1) ."', EscaYear = '". $arr[0] ."', EscaRate = '". $arr[1] ."', EscaAmount = '". $arr[2] ."', EscaTotal = '". $arr[3] ."', ChargeCode = '". $arr[4] ."', UnitID = '". $arr[5] ."';", $connection);
						}
				...
				if($_POST['isAmendment'] == "1"){
					...
					for ($i = 0; $i <= COUNT($Charges) -2; $i++) { 
						$arr = explode("|", $Charges[$i]);
						$getChargeInfo = mysql_fetch_array(mysql_query("SELECT CHARGE_DESC, RATE_TYPE, CHARGE_ID FROM tblref_refcharges WHERE CHARGE_ID = '". $arr[0] ."';", $connection));
						$resInsertCharges = mysql_query("INSERT INTO tbltrans_procharges SET InquiryID = '". $getFirstID['InquiryID'] ."-". $getAmendCount ."', ProposalNum = '". floatval($LastProposalNum[0] + 1) ."', ChargeCode = '". $arr[0] ."', ChargeDesc = '". $getChargeInfo['CHARGE_DESC'] ."', ChargeType = '". $getChargeInfo['RATE_TYPE'] ."', ChargeAmount = '". $arr[1] ."', UnitID = '". $arr[2] ."';", $connection);
					}
					...
					if($_POST['MMS_Module'] != 'inquiry'){
						if($_POST['isProposal'] == "0"){
							...
							for ($i = 0; $i <= COUNT($Charges) -2; $i++) { 
								$arr = explode("|", $Charges[$i]);
								$getChargeInfo = mysql_fetch_array(mysql_query("SELECT CHARGE_DESC, RATE_TYPE, CHARGE_ID FROM tblref_refcharges WHERE CHARGE_ID = '". $arr[0] ."';", $connection));
								$resInsertCharges = mysql_query("INSERT INTO tbltrans_procharges SET InquiryID = '". $_POST['InquiryID'] ."', ProposalNum = '". floatval($LastProposalNum[0] + 1) ."', ChargeCode = '". $arr[0] ."', ChargeDesc = '". $getChargeInfo['CHARGE_DESC'] ."', ChargeType = '". $getChargeInfo['RATE_TYPE'] ."', ChargeAmount = '". $arr[1] ."', UnitID = '". $arr[2] ."';", $connection);
							...
							// INSERT LOGS JONAS 9/30/2019 START
							$arrHeader = ["Proposal", "Leads ID", "Inquiry ID", "Trade ID", "Trade Name", "Merchant Code", "Company ID", "Company Name", "Industry ID", "Process Owner", "Source", "Classification", "Department", "Category", "Mall ID", "Mall Name", "Start Date", "End Date", "Months", "Year", "Escalation Rate", "Year Start", "Year Basis", "Rent Free Construction", "Rent Free Construction Start Date", "is rent VATable?", "Monthly Rent", "Unit", "Charges List", "Requirements", "Permits"];
							...
							for ($i = 0; $i <= COUNT($Escalation2) -2; $i++) { 
								$arr = explode("|", $Escalation2[$i]);
								$resInsertEscalation = mysql_query("INSERT INTO tbltrans_escalation SET InquiryID = '". $_POST['InquiryID'] ."', ProposalNum = '". $_POST['isProposal'] ."', EscaYear = '". $arr[0] ."', EscaRate = '". $arr[1] ."', AccuEscaRate = '". $arr[2] ."', EscaAmount = '". $arr[3] ."', EscaTotal = '". $arr[4] ."';", $connection);
							}

							$resDeleteChargeEscalation = mysql_query("DELETE FROM tbltrans_chargeesca WHERE InquiryID = '". $_POST['InquiryID'] ."' AND ProposalNum = '". $_POST['isProposal'] ."';", $connection);
							$ChargeEscalation = explode("@", $_POST['ChargseEsca']);
							for ($i = 0; $i <= COUNT($ChargeEscalation) -2; $i++) { 
								$arr = explode("|", $ChargeEscalation[$i]);
								$resInsertEscalation = mysql_query("INSERT INTO tbltrans_chargeesca SET InquiryID = '". $_POST['InquiryID'] ."', ProposalNum = '". $_POST['isProposal'] ."', YearStart = '". $arr[0] ."', YearBasis = '". $arr[1] ."', EscaRate = '". $arr[2] ."', ChargeCode = '". $arr[3] ."', UnitID = '". $arr[4] ."';", $connection);
							}

							$resDeleteChargeEscalationBR = mysql_query("DELETE FROM tbltrans_chargeesca_br WHERE InquiryID = '". $_POST['InquiryID'] ."' AND ProposalNum = '". $_POST['isProposal'] ."';", $connection);
							$ChargeEscalationBR = explode("@", $_POST['ChargesEscaBR']);
							for ($i = 0; $i <= COUNT($ChargeEscalationBR) -2; $i++) { 
								$arr = explode("|", $ChargeEscalationBR[$i]);
								$resInsertEscalation = mysql_query("INSERT INTO tbltrans_chargeesca_br SET InquiryID = '". $_POST['InquiryID'] ."', ProposalNum = '". $_POST['isProposal'] ."', EscaYear = '". $arr[0] ."', EscaRate = '". $arr[1] ."', EscaAmount = '". $arr[2] ."', EscaTotal = '". $arr[3] ."', ChargeCode = '". $arr[4] ."', UnitID = '". $arr[5] ."';", $connection);
							}
							...
							if($resDeleteCharges == true){
								...
								for ($i = 0; $i <= COUNT($Charges) -2; $i++) { 
									$arr = explode("|", $Charges[$i]);
									$getChargeInfo = mysql_fetch_array(mysql_query("SELECT CHARGE_DESC, RATE_TYPE, CHARGE_ID FROM tblref_refcharges WHERE CHARGE_ID = '". $arr[0] ."';", $connection));
									$resInsertCharges = mysql_query("INSERT INTO tbltrans_procharges SET InquiryID = '". $_POST['InquiryID'] ."', ProposalNum = '". $_POST['isProposal'] ."', ChargeCode = '". $arr[0] ."', ChargeDesc = '". $getChargeInfo['CHARGE_DESC'] ."', ChargeType = '". $getChargeInfo['RATE_TYPE'] ."', ChargeAmount = '". $arr[1] ."', UnitID = '". $arr[2] ."';", $connection);
									$ChargesList .=  "<br/>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;". $getChargeInfo['CHARGE_ID'] ."&nbsp;<span class='fa fa-angle-right blue'></span>&nbsp;". $getChargeInfo['CHARGE_DESC'];
								}
							}
		...
		case 'fncLoadChargeProposal':
			...
			$res = mysql_query("SELECT ChargeCode, ChargeDesc, ChargeType, ChargeAmount, UnitID FROM tbltrans_procharges WHERE InquiryID = '". $_POST['InquiryID'] ."' AND ProposalNum = '". $_POST['isProposal'] ."';", $connection);
			while($row = mysql_fetch_array($res)){
				...
				$UnitInfo = mysql_fetch_array(mysql_query("SELECT unitname, area, sqm_height, sqm_width FROM tblref_unit WHERE unitID = '". $row['UnitID'] ."';", $connection));
                if(SysLeaseSetup('floorandunitmeasurement') == "Area"){
                	$isArea = floatval($UnitInfo['area']);
                }else{
                	$isArea = floatval($UnitInfo['sqm_height'] * $UnitInfo['sqm_width']);
                ...
                if($getChargeInfo['RATE_TYPE'] == "Persqm"){
            		$ChargeAmount = floatval($row['ChargeAmount']) * floatval($isArea);
            	}else{
            		$ChargeAmount = floatval($row['ChargeAmount']);
            	}

				echo "<tr id='". $row['ChargeCode'] ."' class='Esca-". $row['ChargeCode'] ."-". $row['UnitID'] ."'>
							<td>". $UnitInfo['unitname'] ."</td>
							<td>". number_format($isArea, "2", ".", ",") ." SQM</td>
							<td>". $getChargeInfo['CHARGE_DESC'] ."</td>
							<td>". $Parusa ."</td>
							<td style='text-align: right;' >". number_format($ChargeAmount, "2", ".", ",") ."</td>
							<td class='hide getthisvalue'>". floatval($row['ChargeAmount']) ."</td>
							<td style='text-align: center;z-index: 0;'><button class='btn btn-xs btn-danger isFinal btn-round txtInqDisabled' onclick='$(\".Esca-". $row['ChargeCode'] ."-". $row['UnitID'] ."\").remove(); fncgetPaymentSchedule(); getTotalMonthlyCharges();'><i class='fa fa-trash-o'></i></button></td>
							<td class='hide'>". $row['UnitID'] ."</td>
						</tr>";
			}
		break;
		...
		case 'fncCancelApplication2':
			...
			if( $resUpdateInquiry == true){
				...
				$resUpdateUnit = mysql_query("UPDATE tblref_unit SET status = 'Vacant', TenantID = NULL, TenantName = NULL, startDate = NULL, endDate = NULL WHERE unitid = '". $rowSelectedUnit['UnitID'] ."';", $connection);

				$resInsertRemarks = mysql_query("INSERT INTO tbltrans_remarks(remID, inqID, xremarks, xdate, userID, xsource)VALUES('". createidno('REM', 'tbltrans_remarks', 'remID') ."', '". $_POST['InquiryID'] ."', '". $_POST['Remarks'] ."', '". date(getsysdate()." H:i:s") ."', '". $_SESSION['MMS-UserID'] ."','".$_POST['xsource']."')", $connection);
				
				$resProInfo = mysql_query("SELECT datefrom, dateto, proposalNum FROM tbltrans_proposal WHERE InquiryID = '". $_POST['InquiryID'] ."'", $connection);
				while($rowProInfo = mysql_fetch_array($resProInfo)){

					$resSelectedUnit = mysql_query("SELECT UnitID FROM tbltrans_proposal_unit WHERE InquiryID = '". $_POST['InquiryID'] ."' AND ProposalNum = '". $rowProInfo['proposalNum'] ."';", $connection);
					while($rowSelectedUnit = mysql_fetch_array($resSelectedUnit)){

						$resCheckStatus = mysql_query("SELECT Status, xdate FROM tblunit_statuslogs WHERE UnitID = '". $rowSelectedUnit['UnitID'] ."' AND xdate BETWEEN '". date('Y-m-d', strtotime($rowProInfo['datefrom'])) ."' AND '". date('Y-m-d', strtotime($rowProInfo['dateto'])) ."';", $connection);
						while($rowCheckStatus = mysql_fetch_array($resCheckStatus)){
							if($rowCheckStatus['Status'] == "ForAwarding"){

		                		$resInsertLogs = mysql_query("UPDATE tblunit_statuslogs SET status = 'Vacant' WHERE unitid = '". $rowSelectedUnit['UnitID'] ."' AND xdate = '". date('Y-m-d', strtotime($rowCheckStatus['xdate'])) ."';", $connection);
		...
		case 'fncPreviewProposal':
            ...
            $getTradePrimaryContact = mysql_fetch_array(mysql_query("SELECT LastName, Designation FROM tbltrans_trade_contact_person WHERE TradeID = '". $_POST['TradeID'] ."' AND isActive = '1' AND isPrimary = '1';", $connection));
		...
		case 'fncgetEscalationSched':
			...
			if($inYear >= 1){
				...
				for ($i = $YearStart; $i <= $EndYear; $i += $_POST['YearBasis']) { 
					$Escalation = ($_POST['Rent'] + $ComputedEscalation) * floatval($_POST['EscaRate'] / 100);
					$ComputedEscalation += $Escalation;
					...
					echo 	"<tr>
								...
								<td style='text-align: center; vertical-align: middle;z-index: 0;'>
									<span class='input-icon input-icon-right'>
			                            <input type='text' class='form-control txtEscalation' style='background: transparent !important; border: 0px; text-align: center; color: #393939;' onkeypress='return isNumberKey(event);' onchange='fncGetNewEscalation(\"". $Count ."\");' onkeyup='fncGetNewEscalation(\"". $Count ."\");' value='". $_POST['EscaRate'] ."' id='tdEscaRate". $Count ."'>
			                            <i class='ace-icon fa fa-percent' style='z-index: 0;'></i>
			                        </span>
								</td>
								<td style='text-align: right;' id='tdEscalation". $Count ."'>". number_format($Escalation, 2, '.', ',') ."</td>
								<td style='text-align: right;' id='tdEscalationAmount". $Count ."'>". number_format($_POST['Rent'] + $ComputedEscalation, 2, '.', ',') ."</td>
							</tr>";
					$Count++;
				}
			}
		...
		case 'fncLoadEscalation':
			$Count = 0;
			$resGetEscalation = mysql_query("SELECT EscaYear, EscaRate, EscaAmount, EscaTotal FROM tbltrans_escalation WHERE InquiryID = '". $_POST['InquiryID'] ."' AND ProposalNum = '". $_POST['isProposal'] ."';", $connection);
			while ($rowEscalation = mysql_fetch_array($resGetEscalation)) {
				echo 	"<tr>
							...
							<td style='text-align: center; vertical-align: middle;z-index: 0;'>
								<span class='input-icon input-icon-right'>
		                            <input type='text' class='form-control txtEscalation' style='background: transparent !important; border: 0px; text-align: center; color: #393939;' onkeypress='return isNumberKey(event);' onchange='fncGetNewEscalation(\"". $Count ."\");' onkeyup='fncGetNewEscalation(\"". $Count ."\");' value='". floatval($rowEscalation['EscaRate']) ."' id='tdEscaRate". $Count ."'>
		                            <i class='ace-icon fa fa-percent' style='z-index: 0;'></i>
		                        </span>
							</td>
							<td style='text-align: right;' id='tdEscalation". $Count ."'>". number_format($rowEscalation['EscaAmount'], 2, '.', ',') ."</td>
							<td style='text-align: right;' id='tdEscalationAmount". $Count ."'>". number_format($rowEscalation['EscaTotal'], 2, '.', ',') ."</td>
						</tr>";
				$Count++;
			}
		break;

		case 'fncLoadChargesEscalation':
			$Count = 0;
			$resChargeEscalation = mysql_query("SELECT UnitID, ChargeCode, YearStart, YearBasis, EscaRate FROM tbltrans_chargeesca WHERE InquiryID = '". $_POST['InquiryID'] ."' AND ProposalNum = '". $_POST['isProposal'] ."';", $connection);
			while($rowChargeEscalation = mysql_fetch_array($resChargeEscalation)){
				$UnitInfo = mysql_fetch_array(mysql_query("SELECT unitname, area, sqm_height, sqm_width FROM tblref_unit WHERE unitID = '". $rowChargeEscalation['UnitID'] ."';", $connection));

				if(SysLeaseSetup('floorandunitmeasurement') == "Area"){
                	$isArea = number_format($UnitInfo['area'], "2", ".", ",");
                }else{
                	$isArea = number_format($UnitInfo['sqm_height'] * $UnitInfo['sqm_width'], "2", ".", ",");
                }

                $ChargeInfo = mysql_fetch_array(mysql_query("SELECT ChargeType, ChargeAmount, ChargeDesc FROM tbltrans_procharges WHERE ChargeCode = '". $rowChargeEscalation['ChargeCode'] ."' AND UnitID = '". $rowChargeEscalation['UnitID'] ."';", $connection));

                if($ChargeInfo['ChargeType'] == "Persqm"){
            		$ChargeAmount = floatval($ChargeInfo['ChargeAmount']) * floatval($isArea);
            	}else{
            		$ChargeAmount = floatval($ChargeInfo['ChargeAmount']);
            	}

				echo 	"<tr class='Esca-". $rowChargeEscalation['ChargeCode'] ."-". $rowChargeEscalation['UnitID'] ."' onclick='fncViewChargeEscaBreakdown(\"". $rowChargeEscalation['ChargeCode'] ."\", \"". $rowChargeEscalation['UnitID'] ."\", \"". $Count ."\", \"1\")'>
							<td class='dipindot' style='vertical-align: middle;'>". $UnitInfo['unitname'] ."</td>
							<td class='dipindot' style='vertical-align: middle;'>". $ChargeInfo['ChargeDesc'] ."</td>
							<td class='dipindot'><input type='text' class='form-control txtChargeEscalation txtChargeEscalation1' style='background-color: transparent; border: 0px; text-align: center; color: #393939;' onkeypress='return isNumberKey(event);' onchange='fncViewChargeEscaBreakdown(\"". $rowChargeEscalation['ChargeCode'] ."\", \"". $rowChargeEscalation['UnitID'] ."\", \"". $Count ."\", \"0\");' onkeyup='fncViewChargeEscaBreakdown(\"". $rowChargeEscalation['ChargeCode'] ."\", \"". $rowChargeEscalation['UnitID'] ."\", \"". $Count ."\", \"0\");' value='". floatval($rowChargeEscalation['YearStart']) ."' id='txtChargeEscaYearStart". $Count ."'></td>
							<td class='dipindot'><input type='text' class='form-control txtChargeEscalation txtChargeEscalation2' style='background-color: transparent; border: 0px; text-align: center; color: #393939;' onkeypress='return isNumberKey(event);' onchange='fncViewChargeEscaBreakdown(\"". $rowChargeEscalation['ChargeCode'] ."\", \"". $rowChargeEscalation['UnitID'] ."\", \"". $Count ."\", \"0\");' onkeyup='fncViewChargeEscaBreakdown(\"". $rowChargeEscalation['ChargeCode'] ."\", \"". $rowChargeEscalation['UnitID'] ."\", \"". $Count ."\", \"0\");' value='". floatval($rowChargeEscalation['YearBasis']) ."' id='txtChargeEscaYearBasis". $Count ."'></td>
							<td class='dipindot'>
								<span class='input-icon input-icon-right'>
		                            <input type='text' class='form-control txtChargeEscalation txtChargeEscalation3' style='background-color: transparent; border: 0px; text-align: center; color: #393939;' onkeypress='return isNumberKey(event);' onchange='fncViewChargeEscaBreakdown(\"". $rowChargeEscalation['ChargeCode'] ."\", \"". $rowChargeEscalation['UnitID'] ."\", \"". $Count ."\", \"0\");' onkeyup='fncViewChargeEscaBreakdown(\"". $rowChargeEscalation['ChargeCode'] ."\", \"". $rowChargeEscalation['UnitID'] ."\", \"". $Count ."\", \"0\");' value='". floatval($rowChargeEscalation['EscaRate']) ."' id='txtChargeEscaRate". $Count ."'>
		                            <i class='ace-icon fa fa-percent' style='z-index: 0;'></i>
		                        </span>
							</td>
							<td class='hide'><input type='hidden' value='". $ChargeAmount ."' id='txtChargeEscaAMount". $Count ."'></td>
							<td class='hide'>". $rowChargeEscalation['ChargeCode'] ."</td>
							<td class='hide'>". $rowChargeEscalation['UnitID'] ."</td>
						</tr>";
				$Count++;
			}
		...
		case 'fncLoadChargesEscalationBR':
			$resChargeEscalation = mysql_query("SELECT UnitID, ChargeCode FROM tbltrans_chargeesca WHERE InquiryID = '". $_POST['InquiryID'] ."' AND ProposalNum = '". $_POST['isProposal'] ."';", $connection);
			while($rowChargeEscalation = mysql_fetch_array($resChargeEscalation)){
				echo 	"<div style='height: 40vh;' class='divChargesEscalation hide Esca-". $rowChargeEscalation['ChargeCode'] ."-". $rowChargeEscalation['UnitID'] ."' id='div-". $rowChargeEscalation['ChargeCode'] ."-". $rowChargeEscalation['UnitID'] ."'>
							<table class='table table-bordered table-striped fixTable'>
						        <thead>
						            <tr>
						                <th style='width: 25%;vertical-align: middle;'>Year</th>
						        ...
						        <tbody id='tbody_". $rowChargeEscalation['ChargeCode'] ."_". $rowChargeEscalation['UnitID'] ."' class='tbodyChargesEscalation'>";
						        	$Count = 0;
									$resChargeEscalationBR = mysql_query("SELECT UnitID, ChargeCode, EscaYear, EscaRate, EscaAmount, EscaTotal FROM tbltrans_chargeesca_br WHERE InquiryID = '". $_POST['InquiryID'] ."' AND ProposalNum = '". $_POST['isProposal'] ."' AND UnitID = '". $rowChargeEscalation['UnitID'] ."' AND ChargeCode = '". $rowChargeEscalation['ChargeCode'] ."'", $connection);
									while($rowChargeEscalationBR = mysql_fetch_array($resChargeEscalationBR)){
										if($Count == 0){
											$PrevTotal = $rowChargeEscalationBR['EscaTotal'] - $rowChargeEscalationBR['EscaAmount'];
										}
										echo 	"<tr>
													<td style='text-align: center; vertical-align: middle;'>". $rowChargeEscalationBR['EscaYear'] ."</td>
													<td style='text-align: center; vertical-align: middle;z-index: 0;'>
														<span class='input-icon input-icon-right'>
								                            <input type='text' class='form-control txtEscalation' style='background: transparent !important; border: 0px; text-align: center; color: #393939;' onkeypress='return isNumberKey(event);' onchange='fncGetNewEscalationBreakdown(\"". $rowChargeEscalationBR['ChargeCode'] ."\", \"". $rowChargeEscalationBR['UnitID'] ."\", \"". $Count ."\", \"". $PrevTotal ."\");' onkeyup='fncGetNewEscalationBreakdown(\"". $rowChargeEscalationBR['ChargeCode'] ."\", \"". $rowChargeEscalationBR['UnitID'] ."\", \"". $Count ."\", \"". $PrevTotal ."\");' value='". floatval($rowChargeEscalationBR['EscaRate']) ."' id='tdEscaRate-". $rowChargeEscalationBR['ChargeCode'] ."-". $rowChargeEscalationBR['UnitID'] ."-". $Count ."'>
								                            <i class='ace-icon fa fa-percent' style='z-index: 0;'></i>
								                        </span>
													</td>
													<td style='text-align: right;' id='tdEscalation-". $rowChargeEscalationBR['ChargeCode'] ."-". $rowChargeEscalationBR['UnitID'] ."-". $Count ."'>". number_format($rowChargeEscalationBR['EscaAmount'], 2, '.', ',') ."</td>
													<td style='text-align: right;' id='tdEscalationAmount-". $rowChargeEscalationBR['ChargeCode'] ."-". $rowChargeEscalationBR['UnitID'] ."-". $Count ."'>". number_format($rowChargeEscalationBR['EscaTotal'], 2, '.', ',') ."</td>
													<td class='hide'>". $rowChargeEscalationBR['ChargeCode'] ."</td>
													<td class='hide'>". $rowChargeEscalationBR['UnitID'] ."</td>
												</tr>";
										$Count++;
									}
						echo    "</tbody>
