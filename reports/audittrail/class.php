...
include "../../connect.php";
	switch ($_POST['form']) {
		case 'displayaudittrail':
			...
			$sql = "SELECT username, mytime, mydate, module, remarks, xinfo, userid FROM tbllogs_per_trans WHERE mydate BETWEEN '". date('Y-m-d', strtotime($_POST['dateFrom'])) ."' AND '". date('Y-m-d', strtotime($_POST['dateTo'])) ."' AND username LIKE '%". $_POST['txtaudittrail'] ."%' ". $filterbymodule ."  GROUP BY mytime ORDER BY ". $_POST['AudittrailSortBy'] ." ". $_POST['AudittrailSortType'] ." LIMIT ". $limit .",20;";
            // echo "SELECT username, mytime, mydate, module, remarks, xinfo, userid FROM tbllogs_per_trans WHERE mydate BETWEEN '". date('Y-m-d', strtotime($_POST['dateFrom'])) ."' AND '". date('Y-m-d', strtotime($_POST['dateTo'])) ."' AND username LIKE '%". $_POST['txtaudittrail'] ."%' ". $filterbymodule ."  GROUP BY logid ORDER BY ". $_POST['AudittrailSortBy'] ." ". $_POST['AudittrailSortType'] ." LIMIT ". $limit .",20";
