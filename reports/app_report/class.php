<?php
	...
	switch($_POST['form']){
        case 'showappreport':
            $page = $_POST['page'];
            $limit = ($page-1) * 20;
            $sql = "SELECT Application_ID, applicationDate, Company_Name, Trade_Name, Status, datefrom, dateto, Inquiry_ID FROM tbltrans_inquiry WHERE applicationDate BETWEEN '". date('Y-m-d', strtotime($_POST['dateFrom'])) ."' AND '". date('Y-m-d', strtotime($_POST['dateTo'])) ."' AND Status = 'Pending' ". getMallAccess("Mall_ID", "AND") ." GROUP BY Application_ID ORDER BY ". $_POST['ApplicationSortBy'] ." ". $_POST['ApplicationSortType'] ." LIMIT ". $limit .",20;";
