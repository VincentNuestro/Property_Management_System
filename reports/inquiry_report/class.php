<?php
	...
	switch($_POST['form']){
		case 'showinquiryreport':
			$page = $_POST['page'];
        	$limit = ($page-1) * 20;
			$sql = "SELECT Inquiry_ID, Company_Name, Trade_Name, UnitType, date_inquired, datefrom, dateto FROM tbltrans_inquiry WHERE (Application_ID = '') AND date_inquired BETWEEN '".date('Y-m-d', strtotime($_POST['dateFrom'])) ."' AND '".date('Y-m-d', strtotime($_POST['dateTo'])) ."' ". getMallAccess("Mall_ID", "AND") ." GROUP BY date_inquired ORDER BY ". $_POST['InquirySortBy'] ." ". $_POST['InquirySortType'] ." LIMIT ".$limit.",20;";
