<script type="text/javascript">
	...
	$(function(){
		...
		// ruth
		$('#printviolation').css("display", "block");
		// ruth
		$('#tblviolation').html("<tr><td colspan='8' style='text-align: center;'>No Data Found...</td></tr>");
	...
	function showviolation(){
		...
		$("#showviolation").css("display", "block");
 ...
 //    	$("#txtTSRPage").val("1");
	...
	function dbviolation(){
		...
		$.ajax ({
			type: 'POST',
			url: 'reports/violationrep/class.php',
			data: 'key=' + key + '&page=' + page + '&MallID=' + MallID + '&form=dbviolation',
			success: function(data){
				...
				fncTSREntries('dbviolation');
				fncTSRPage('dbviolation');
			}
		})
	}

	function AutoConsolidateviolation(mallid){
		...
		$.ajax ({
			type: 'POST',
			url: 'reports/violationrep/class.php',
			data: 'tenant=' + tenant + '&datefrom=' + datefrom + '&dateto=' + dateto + '&mallid=' + mallid + '&form=AutoConsolidateviolation',
			success: function (data) {
				alert(data);
				if(data == 1){
					setTimeout(function(){
						showmodal("alert", "Violation Report successfully exported.", "", null, "", null, "0");
					}, 500)
				}else{
					setTimeout(function(){
						showmodal("alert", "Failed to export Violation Report.", "", null, "", null, "1");
					}, 500)
				}
			}
		})
