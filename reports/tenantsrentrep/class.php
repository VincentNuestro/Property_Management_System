<?php
	...
	switch ($_POST['form']) {
		...
		case 'dbrent':
			...
			$TenantInfo = mysql_fetch_array(mysql_query("SELECT tradename, companyname, mallID, tradeID, tenanttype, revpercent, merchant_code, companyID, inqID FROM tbltrans_tenants WHERE tradename = '". $_POST['key'] ."';", $connection));
			$inquiryid = mysql_fetch_array(mysql_query("SELECT inquiryID FROM tbltrans_inquiry_unit WHERE inquiryID = '".$TenantInfo[8]."';", $connection));
			$query = mysql_query("SELECT unitname, buildingname, typeofbusiness, totalamountunitsetup, tenantID, tenantName FROM tblref_unit WHERE tenantName LIKE '%". $_POST['key']."%';",  $connection);
	
			while($row = mysql_fetch_array($query)){
				echo 	"<tr>
							<td>". $TenantInfo['tradename']."</td>
							<td>". $inquiryid['inquiryID'] ."</td>
							<td>". $row[0]."</td>
							<td>". $row[1]."</td>
							<td>". $row[2]."</td>
		...
		case 'printrent':
			$TenantInfo = mysql_fetch_array(mysql_query("SELECT tradename, companyname, mallID, tradeID, tenanttype, revpercent, merchant_code, companyID, inqID FROM tbltrans_tenants WHERE tradename = '". $_POST['key'] ."';", $connection));
			$inquiryid = mysql_fetch_array(mysql_query("SELECT inquiryID FROM tbltrans_inquiry_unit WHERE inquiryID = '".$TenantInfo[8]."';", $connection));
			$query = mysql_fetch_array(mysql_query("SELECT unitname, buildingname, typeofbusiness, totalamountunitsetup, tenantID, tenantName FROM tblref_unit WHERE tenantName LIKE '%". $_POST['key']."%';",  $connection));
					?>  
					<style type="text/css">
						#hr{
					...
					<center> <h2> <b>Tenant Rent Report</b> </h2> </center>
					...
					<table  style='width: 100%;' cellspacing='0' cellpadding='0'>
						<thead>
							...
							<th style="border: 1px solid black; text-align: center; color: white; padding:3px !important;"> Inquiry ID </th>
					...
					// while($row = mysql_fetch_array($res)){
						...
						<tr id='<?php echo $row[0]; ?>'>
							...
							<td style="border: 1px solid black; padding:5px;"><?php echo $inquiryid[0]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $query[0]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $query[1]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $query[2]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $query[3]; ?></td>
						</tr>
						<?php
					// }
					?></tbody></table><?php
		...
		case 'AutoConsolidaterent':
			// $TenantInfo = mysql_fetch_array(mysql_query("SELECT tradename FROM tbltrans_tenants WHERE TenantID = '". $_POST['tenant'] ."';", $connection));
			// $inquiryid = mysql_fetch_array(mysql_query("SELECT inquiryID FROM tbltrans_inquiry_unit;", $connection));
			// $rent = mysql_query("SELECT unitname, buildingname, typeofbusiness, totalamountunitsetup, tenantID, tenantName FROM tblref_unit WHERE TenantID = '". $_POST['tenant'] ."';",  $connection);

			$TenantInfo = mysql_fetch_array(mysql_query("SELECT tradename, companyname, mallID, tradeID, tenanttype, revpercent, merchant_code, companyID, inqID FROM tbltrans_tenants WHERE tradename = '". $_POST['key'] ."';", $connection));
			$inquiryid = mysql_fetch_array(mysql_query("SELECT inquiryID FROM tbltrans_inquiry_unit WHERE inquiryID = '".$TenantInfo[8]."';", $connection));
			$query = mysql_fetch_array(mysql_query("SELECT unitname, buildingname, typeofbusiness, totalamountunitsetup, tenantID, tenantName FROM tblref_unit WHERE tenantName LIKE '%". $_POST['key']."%';",  $connection));

			$data = "Store Name,InquiryID,Unit Name,Building Name,Type of Business,Total Amount\r\n";
			// while($rowrent = mysql_fetch_array($query)){
				$data .= str_replace(",","",$TenantInfo[0]).",".$inquiryid[0].",".$query[0].",".$query[1].",".$query[2].",".$query[3]."'\r\n";
			// }
			...
			$file = $syspath."/TenantRent".date('mdY his').".csv";
			chmod($file, 0777);
			if (file_put_contents($file, $data)){
				create_logs_per_transaction('exported a Tenant Rent to excel', 'Tenant Rent Report', '', '', 'EXPORT', '');
				echo 1;
			}else{
				echo 0;
			}
