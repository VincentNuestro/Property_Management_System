<script type="text/javascript">
	...
	$(function(){
		...
		// ruth
		$('#printrent').css("display", "block");
		$('#tblrent').html("<tr><td colspan='6' style='text-align: center;'>No Data Found...</td></tr>");
		// ruth
 ...
 //    	$("#txtTSRPage").val("1");
	...
	function dbrent(){
		...
		$.ajax ({
			type: 'POST',
			url: 'reports/tenantsrentrep/class.php',
			data: 'key=' + key + '&page=' + page + '&MallID=' + MallID + '&form=dbrent',
			success: function(data){
				if(data != ""){
		          	$("#tblrent").html(data);
		          	alert(data);
				...
				fncTSREntries('dbrent');
				fncTSRPage('dbrent');
			}
		})
	}

	function AutoConsolidaterent(mallid){
		...
		$.ajax ({
			type: 'POST',
			url: 'reports/tenantsrentrep/class.php',
			data: 'tenant=' + tenant + '&datefrom=' + datefrom + '&dateto=' + dateto + '&mallid=' + mallid + '&form=AutoConsolidaterent',
			success: function (data) {
				alert(data);
				if(data == 1){
					setTimeout(function(){
						showmodal("alert", "Tenant Rent successfully exported.", "", null, "", null, "0");
					}, 500)
				}else{
					setTimeout(function(){
						showmodal("alert", "Failed to export Tenant Rent.", "", null, "", null, "1");
					}, 500)
				}
			}
		})
	...
	// 	$.ajax ({
	// 		type: 'POST',
	// 		url: 'reports/tenantsrentsrep/class.php',
