<script type="text/javascript">
	...
	$(function(){
		...
		// ruth
		$('#printcomplaint').css("display", "block");
		// ruth
		$('#tblrent').html("<tr><td colspan='9' style='text-align: center;'>No Data Found...</td></tr>");
	...
	function showcomplaint(){
		...
		$("#showcomplaint").css("display", "block");
 ...
 //    	$("#txtTSRPage").val("1");
	...
	function dbcomplaint(){
		...
		$.ajax ({
			type: 'POST',
			url: 'reports/complaintrep/class.php',
			data: 'key=' + key + '&page=' + page + '&MallID=' + MallID + '&form=dbcomplaint',
			success: function(data){

				if(data != ""){
		          	$("#tblcomplaint").html(data);
		          	// alert(data);

		        }else{
		          	$("#tblcomplaint").html("<tr><td colspan='9' style='text-align: center;'>No Data Found...</td></tr>");
		        }
				fncTSREntries('dbcomplaint');
				fncTSRPage('dbcomplaint');
			}
		})
	}

	function AutoConsolidatecomplaint(mallid){
		...
		$.ajax ({
			type: 'POST',
			url: 'reports/complaintrep/class.php',
			data: 'tenant=' + tenant + '&datefrom=' + datefrom + '&dateto=' + dateto + '&mallid=' + mallid + '&form=AutoConsolidatecomplaint',
			success: function (data) {
				alert(data);
				if(data == 1){
					setTimeout(function(){
						showmodal("alert", "Complaint Report successfully exported.", "", null, "", null, "0");
					}, 500)
				}else{
					setTimeout(function(){
						showmodal("alert", "Failed to export Complaint Report.", "", null, "", null, "1");
					}, 500)
				}
			}
		})
		...
		$.ajax ({
			...
			success: function(data){
				if(data != ""){
		          	...
		          	$("#tblpayment").html("<tr><td colspan='6' style='text-align: center;'>No Data Found...</td></tr>");
		        }
