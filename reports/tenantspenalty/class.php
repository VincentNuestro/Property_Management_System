...
<?php
	...
	switch ($_POST['form']) {
		...
		case 'dbpenalty':
			...
			$sql2 = mysql_query("SELECT a.tenantid, a.reference, a.xcode, a.amount, a.vatamount, a.balance, a.xdate, b.tradename, a.id, a.isPosted, a.description, a.totalamount, a.isPenalty FROM tbltransaction AS a INNER JOIN tbltrans_tenants AS b ON a.tenantid = b.tenantid WHERE b.tradename LIKE '%". $_POST['key'] ."%' AND a.isPenalty = '1' AND a.xdate BETWEEN '".date("Y-m-d", strtotime($_POST['DateFrom']))."' AND '".date("Y-m-d", strtotime($_POST['DateTo']))."';", $connection);
			while($row = mysql_fetch_array($sql2)){
				echo 	"<tr>
							...
							<td style='text-align: right;'>". $row['reference']."</td>
						</tr>";	
					}
		break;
		...
		case 'printpenalty':
			$res = mysql_fetch_array(mysql_query("SELECT a.tenantid, a.reference, a.xcode, a.amount, a.vatamount, a.balance, a.xdate, b.tradename, a.id, a.isPosted, a.description, a.totalamount, a.isPenalty FROM tbltransaction AS a INNER JOIN tbltrans_tenants AS b ON a.tenantid = b.tenantid WHERE b.tradename LIKE '%". $_POST['key'] ."%' AND a.isPenalty = '1' AND a.xdate BETWEEN '".date("Y-m-d", strtotime($_POST['DateFrom']))."' AND '".date("Y-m-d", strtotime($_POST['DateTo']))."';", $connection));
					?>  
					<style type="text/css">
						#hr{
					...
					<center> <h2> <b>Tenants Penalty Report</b> </h2> </center>
					...
					<table  style='width: 100%;' cellspacing='0' cellpadding='0'>
						<thead>
							...
							<th style="border: 1px solid black; text-align: center; color: white; padding:3px !important;"> Description </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:3px !important;"> Amount </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:3px !important;"> Vatable Amount </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:3px !important;"> Balance </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:5px !important;"> Total Amount </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:5px !important;"> Reference </th>
						</thead>
						<tbody>
					...
					// while($row = mysql_fetch_array($res)){
						...
						<tr id='<?php echo $row[0]; ?>'>
							...
							<td style="border: 1px solid black; padding:5px;"><?php echo $res[10]; ?></td>
							...
							<td style="border: 1px solid black; padding:5px;"><?php echo $res[11]; ?></td>
		...
		$penalty = mysql_fetch_array(mysql_query("SELECT a.tenantid, a.reference, a.xcode, a.amount, a.vatamount, a.balance, a.xdate, b.tradename, a.id, a.isPosted, a.description, a.totalamount, a.isPenalty FROM tbltransaction AS a INNER JOIN tbltrans_tenants AS b ON a.tenantid = b.tenantid WHERE b.tradename LIKE '%". $_POST['tenant'] ."%' AND a.isPenalty = '1' AND a.xdate BETWEEN '".date("Y-m-d", strtotime($_POST['DateFrom']))."' AND '".date("Y-m-d", strtotime($_POST['DateTo']))."';", $connection));
			$data = "Store Name,Transaction Date,Description,Amount,Vatable Amount,Balance,Total Amount,Reference\r\n";
			// while($rowpenalty = mysql_fetch_array($penalty)){
				$data .= str_replace(",","",$penalty[7]).",".$penalty[6].",".$penalty[10].",".$penalty[3].",".$penalty[5].",".$penalty[5].",".$penalty[11].",".$penalty[1].",\r\n";
			// }
			...
			$file = $syspath."/TenantPenalty".date('mdY his').".csv";
			chmod($file, 0777);
			if (file_put_contents($file, $data)){
				create_logs_per_transaction('exported a tenant penalty report to excel', 'Tenants Penalty Report', '', '', 'EXPORT', '');
				echo 1;
			}else{
				echo 0;
			}
