<script type="text/javascript">
	...
	$(function(){
		// ruth
		$('#printpenalty').css("display", "block");
		// ruth
		...
		// showpenalty(); $('#printpenalty').css('display','block');
	});
	...
	function showpenalty(){
		...
		$("#showpenalty").css("display", "block");
		$("#pageSales").css("display", "block");
    	$("#txtTSRPage").val("1");
		$('#tblpenalty').html("<tr><td colspan='7' style='text-align: center;'>No Data Found...</td></tr>");
	}
	...
	function showPayment(){
    	...
    	$("#txtTSRPage").val("1");
		$("#tblpayment").html("<tr><td colspan='7' style='text-align: center;'>No Data Found...</td></tr>");
	}
	...
	function dbpenalty(){
		...
		$.ajax ({
			type: 'POST',
			url: 'reports/tenantspenalty/class.php',
			data: 'key=' + key + '&DateFrom=' + DateFrom + '&DateTo=' + DateTo + '&page=' + page + '&MallID=' + MallID + '&form=dbpenalty',
			success: function(data){
				// if(data != ""){
		          	$("#tblpenalty").html(data);
		          	// alert(data);

		        // }
		        // else{
		        //   	$("#tblcharges").html("<tr><td colspan='10' style='text-align: center;'>No Data Found...</td></tr>");
		        // }
				fncTSREntries('dbpenalty');
				fncTSRPage('dbpenalty');
			}
		});
	}
	...
	function dbPayment(){
		...
		$.ajax ({
			...
			success: function(data){
				// if(data != ""){
		          	$("#tblpayment").html(data);
		        // }
		        // else{
		        //   	$("#tblpayment").html("<tr><td colspan='8' style='text-align: center;'>No Data Found...</td></tr>");
		        // }
	...
	function AutoConsolidatepenalty(mallid){
		...
		$.ajax ({
			type: 'POST',
			url: 'reports/tenantspenalty/class.php',
			data: 'tenant=' + tenant + '&mallid=' + mallid + '&DateFrom=' + DateFrom + '&DateTo=' + DateTo + '&form=AutoConsolidatepenalty',
			success: function (data) {
				alert(data);
				if(data == 1){
					setTimeout(function(){
						showmodal("alert", "Tenant Penalty successfully exported.", "", null, "", null, "0");
					}, 500)
				}else{
					setTimeout(function(){
						showmodal("alert", "Failed to export Tenant Penalty.", "", null, "", null, "1");
					}, 500)
				}
			}
		})
	...
	function fncTSREntries(ReportType){
	    ...
	    $.ajax({
	        type: 'POST',
	        url: 'reports/tenantspenalty/class.php',
	        data: 'key=' + key + '&page=' + page + '&MallID=' + MallID + '&ReportType=' + ReportType + '&form=fncTSREntries',
	        success: function(data){
	...
	function fncTSRPage(ReportType){
	    ...
	    $.ajax({
	        type: 'POST',
	        url: 'reports/tenantspenalty/class.php',
	        data: 'key=' + key + '&page=' + page + '&MallID=' + MallID + '&ReportType=' + ReportType + '&form=fncTSRPage',
	        success: function(data){
...
// ruth
	...
	// function printtenantsalesreporthourly(MallID){
	// 	var datefrom = $("#dateFromList").val();
	// 	var dateto = $("#dateToList").val();
 //        var mallid = $("#txtTSRMall").val();
 //        var tenant = $("#txtSearchTenantName").val();
 ...
 //            	type: 'POST',
 //            	url: 'reports/tenantsalesreport/class.php',
 //            	data: '&tenant=' + tenant + '&dateFrom=' + datefrom + '&dateTo=' + dateto + '&form=printtenantsalesreporthourly',
 //            	success:function(data){
 //            		// alert(data);
 //                    var arr = data.split("|");
 //                    // textsadesign
