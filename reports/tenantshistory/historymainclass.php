<?php
    ...
    switch ($_POST['form']) {
            case 'tblstorename':
                $sql = "SELECT TenantID, mallID, inqID, tradeID, tradename, CompanyID, companyname, unitID, unitname, datefrom, dateto, Status, noofmonths, noofdays, monthly_dues, tenanttype, owner_firstname, owner_midname, owner_lastname FROM tbltrans_tenants WHERE tradename LIKE '%".$_POST['srchhistory']."%' ". getMallAccess("mallID", "AND") ." GROUP BY TenantID ORDER BY ". $_POST['TenantHistorySortBy'] ." ". $_POST['TenantHistorySortType'] ."" ;
