<?php
	...
	switch ($_POST['form']) {
		...
		case 'dbcharges':
			...
			$sql = mysql_query("SELECT a.xdate, a.paymenttype, a.qty, a.reference, a.amount, a.vatamount, a.totalamount, a.paymentamount, a.balance, b.tradename FROM tbltransaction AS a LEFT JOIN tbltrans_tenants AS b ON a.TenantID = b.TenantID WHERE a.paymenttype = '' AND a.xdate BETWEEN '".date("Y-m-d", strtotime($_POST['DateFrom']))."' AND '".date("Y-m-d", strtotime($_POST['DateTo']))."' AND b.tradename LIKE '%". $_POST['key'] ."%' ORDER BY a.xdate DESC;",  $connection);
		...
		case 'printcharges':
			$TenantInfo = mysql_fetch_array(mysql_query("SELECT tradename, companyname, mallID, tradeID, tenanttype, revpercent, merchant_code, companyID FROM tbltrans_tenants WHERE TenantID = '". $_POST['key'] ."';", $connection));
			$res = mysql_query("SELECT a.xdate, a.paymenttype, a.qty, a.reference, a.amount, a.vatamount, a.totalamount, a.paymentamount, a.balance, b.tradename FROM tbltransaction AS a LEFT JOIN tbltrans_tenants AS b ON a.TenantID = b.TenantID WHERE a.paymenttype = '' AND a.xdate BETWEEN '".date("Y-m-d", strtotime($_POST['DateFrom']))."' AND '".date("Y-m-d", strtotime($_POST['DateTo']))."' AND b.tradename LIKE '%". $_POST['key'] ."%' ORDER BY a.xdate DESC;",  $connection);

			// $TenantInfo = mysql_fetch_array(mysql_query("SELECT tradename FROM tbltrans_tenants WHERE TenantID = '". $_POST['tenant'] ."';", $connection));
			// $res = mysql_fetch_array(mysql_query("SELECT xdate, paymenttype, qty, reference, amount, vatamount, totalamount, paymentamount, balance FROM tbltransaction WHERE tenantid = '". $_POST['tenant'] ."' AND paymenttype = '' AND xdate BETWEEN '".date("Y-m-d", strtotime($_POST['datefrom']))."' AND '".date("Y-m-d", strtotime($_POST['dateto']))."' ORDER BY xdate DESC;", $connection));
					?>  
					<style type="text/css">
						#hr{
					...
					<center> <h2> <b>Tenant Charges Report</b> </h2> </center>
					...
					<table  style='width: 100%;' cellspacing='0' cellpadding='0'>
						<thead>
							...
							<th style="border: 1px solid black; text-align: center; color: white; padding:3px !important;"> Billing Type </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:3px !important;"> Charges </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:3px !important;"> Reference </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:3px !important;"> Amount </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:5px !important;"> VAT </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:5px !important;"> Total Amount </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:5px !important;"> Paid Amount </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:5px !important;"> Balance </th>
						</thead>
						<tbody>
					...
					// while($row = mysql_fetch_array($res)){
						...
						<tr id='<?php echo $row[0]; ?>'>
							<td style="border: 1px solid black; padding:5px;"><?php echo $TenantInfo[9]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $res[0]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $res[1]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $res[2]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $res[3]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $res[4]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $res[5]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $res[6]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $res[7]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $res[8]; ?></td>
						</tr>
						<?php
					// }
					?></tbody></table><?php
		...
		case 'AutoConsolidatecharges':
			$TenantInfo = mysql_fetch_array(mysql_query("SELECT tradename FROM tbltrans_tenants WHERE TenantID = '". $_POST['tenant'] ."';", $connection));
			$charges = mysql_query("SELECT xdate, paymenttype, qty, reference, amount, vatamount, totalamount, paymentamount, balance FROM tbltransaction WHERE tenantid = '". $_POST['tenant'] ."' AND paymenttype = '' AND xdate BETWEEN '".date("Y-m-d", strtotime($_POST['datefrom']))."' AND '".date("Y-m-d", strtotime($_POST['dateto']))."' ORDER BY xdate DESC;", $connection);
			$data = "Store Name,Transaction Date,Billing Type,Charges,Reference,Amount,VAT,Total Amount,Paid Amount,Balance\r\n";
			while($rowcharges = mysql_fetch_array($charges)){
				$data .= str_replace(",","",$TenantInfo[0]).",".$rowcharges[0].",".$rowcharges[1].",".$rowcharges[2].",".$rowcharges[3].",".$rowcharges[4].",".$rowcharges[5].",".$rowcharges[6].",".$rowcharges[7].",".$rowcharges[8].",".$rowcharges[9].",\r\n";
			}
			...
			$file = $syspath."/TenantCharges".date('mdY his').".csv";
			chmod($file, 0777);
			if (file_put_contents($file, $data)){
				create_logs_per_transaction('exported a tenant charges report to excel', 'Tenant Charges Report', '', '', 'EXPORT', '');
				echo 1;
			}else{
				echo 0;
			}
		...
		case 'dbPayment':
			...
			$TenantInfo = mysql_fetch_array(mysql_query("SELECT tradename, companyname, mallID, tradeID, tenanttype, revpercent, merchant_code, companyID FROM tbltrans_tenants WHERE tradename LIKE '%". $_POST['key'] ."%';", $connection));
			$sql = mysql_query("SELECT xdate, description, amount, paymenttype, balance, reference FROM tbltransaction WHERE paymenttype = '' AND xdate BETWEEN '".date("Y-m-d", strtotime($_POST['DateFrom']))."' AND '".date("Y-m-d", strtotime($_POST['DateTo']))."' AND tenantID LIKE '%". $_POST['key'] ."%' ORDER BY xdate DESC;", $connection);
			while($row = mysql_fetch_array($sql)){
				echo 	"<tr>
							<td>". $TenantInfo[0]."</td>
							<td>". date('m/d/Y', strtotime($row['xdate'])) ."</td>
							...
							<td style='text-align: center;'>". number_format($row['amount'], "2", ".", ",") ."</td>
							<td style='text-align: right;'>". number_format($row['paymenttype'], "2", ".", ",") ."</td>
							<td style='text-align: right;'>". number_format($row['balance'], "2", ".", ",") ."</td>
							<td>". $row['reference'] ."</td>	
						</tr>";	
			}
		...
		case 'printpayment':
			$TenantInfo = mysql_fetch_array(mysql_query("SELECT tradename, companyname, mallID, tradeID, tenanttype, revpercent, merchant_code, companyID FROM tbltrans_tenants WHERE TenantID = '". $_POST['tenant'] ."';", $connection));
			$sql = mysql_fetch_array(mysql_query("SELECT xdate, description, amount, paymenttype, balance, reference FROM tbltransaction WHERE tenantid = '". $_POST['tenant'] ."' AND paymenttype = '' AND xdate BETWEEN '".date("Y-m-d", strtotime($_POST['datefrom']))."' AND '".date("Y-m-d", strtotime($_POST['dateto']))."' ORDER BY xdate DESC;", $connection));
					?>  
					<style type="text/css">
						#hr{
							background: black !important; padding:2px;
					...
					<center> <h2> <b>Tenant Payment Report</b> </h2> </center>
					<table style='width:100%;'>
						<tr>
							<td id='hr'> </td>
						</tr>
					...
					<table  style='width: 100%;' cellspacing='0' cellpadding='0'>
						<thead>
							<th style="border: 1px solid black; text-align: center; color: white; padding:3px !important;"> Store Name </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:5px !important;"> Inquiry ID </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:3px !important;"> Unit ID </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:3px !important;"> Unit Name </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:3px !important;"> Building Name </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:3px !important;"> Type of Business </th>
							<th style="border: 1px solid black; text-align: center; color: white; padding:5px !important;"> Total Amount </th>
						</thead>
						<tbody>
					...
					// while($row = mysql_fetch_array($res)){
						?>
						<tr id='<?php echo $row[0]; ?>'>
							<td style="border: 1px solid black; padding:5px;"><?php echo $TenantInfo[0]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $sql[0]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $sql[1]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $sql[2]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $sql[3]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $sql[4]; ?></td>
							<td style="border: 1px solid black; padding:5px;"><?php echo $sql[5]; ?></td>
						</tr>
						<?php
					// }
					?></tbody></table><?php
		...
		case 'AutoConsolidatepayment':
			$TenantInfo = mysql_fetch_array(mysql_query("SELECT tradename FROM tbltrans_tenants WHERE TenantID = '". $_POST['tenant'] ."';", $connection));
			$payment = mysql_query("SELECT xdate, description, amount, paymenttype, balance, reference FROM tbltransaction WHERE tenantid = '". $_POST['tenant'] ."' AND paymenttype = '' AND xdate BETWEEN '".date("Y-m-d", strtotime($_POST['datefrom']))."' AND '".date("Y-m-d", strtotime($_POST['dateto']))."' ORDER BY xdate DESC;", $connection);
			$data = "Store Name,Inquiry ID,Unit ID,Unit Name,Building Name,Type of Business,Total Amount\r\n";
			while($rowpayment = mysql_fetch_array($payment)){
				$data .= str_replace(",","",$TenantInfo[0]).",".$rowpayment[0].",".$rowpayment[1].",".$rowpayment[2].",".$rowpayment[3].",".$rowpayment[4].",".$rowpayment[5].",".$rowpayment[6].",\r\n";
			}
			$filepath = mysql_fetch_array(mysql_query("SELECT filepath FROM tblsys_setup", $connection));
			$syspath = str_replace("\\", "/", $filepath[0]);
			if (!file_exists($syspath)){
			...
			$file = $syspath."/TenantPayment".date('mdY his').".csv";
			chmod($file, 0777);
			if (file_put_contents($file, $data)){
				create_logs_per_transaction('exported a Tenant Paymet to excel', 'Tenant Payment Report', '', '', 'EXPORT', '');
				echo 1;
			}else{
				echo 0;
			}
