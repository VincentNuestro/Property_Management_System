	...
	<div class="col-md-12">
		<div class="row form-group" style="margin-bottom: 0px;">
			...
			<div class="col-md-2" style="padding-bottom: 5px;padding-left: 0px;">
				<div class='btn-group' id="printcharges" style="display: none;">
			       <button class='btn btn-primary btn-round btn-sm' onclick="$arr = [];$arr.push( 'page='+$('#txtTSRPage').val() );$arr.push( 'tenant='+$('#txtSearchTenantName').val() );$arr.push( 'datefrom='+$('#dateFromList').val() );$arr.push( 'dateto='+$('#dateToList').val() );$arr.push( 'mallid='+$('#txtTSRMall').val() );getheaderprint('<?php echo $_SESSION['MMS-Designation']; ?>','printcharges',JSON.stringify($arr));" data-placement="bottom">Print Charges</button> 
			        <button data-toggle='dropdown' class='btn dropdown-toggle btn-primary btn-round btn-sm'>
			            <span class='ace-icon fa fa-caret-down icon-only'></span>
			        </button>
			        <ul class='dropdown-menu pull-right'>
			            <li>
			                <a onclick="AutoConsolidatecharges('<?php echo $_SESSION['MMS-Designation']; ?>');">Export</a>
			            </li>
					</ul>
				</div>

				<div class='btn-group' id="printpayment" style="display: none;">
			       <button class='btn btn-primary btn-round btn-sm' onclick="$arr = [];$arr.push( 'page='+$('#txtTSRPage').val() );$arr.push( 'tenant='+$('#txtSearchTenantName').val() );$arr.push( 'datefrom='+$('#dateFromList').val() );$arr.push( 'dateto='+$('#dateToList').val() );$arr.push( 'mallid='+$('#txtTSRMall').val() );getheaderprint('<?php echo $_SESSION['MMS-Designation']; ?>','printpayment',JSON.stringify($arr));" data-placement="bottom">Print Payment</button> 
			        <button data-toggle='dropdown' class='btn dropdown-toggle btn-primary btn-round btn-sm'>
			            <span class='ace-icon fa fa-caret-down icon-only'></span>
			        </button>
			        <ul class='dropdown-menu pull-right'>
			            <li>
			                <a onclick="AutoConsolidatepayment('<?php echo $_SESSION['MMS-Designation']; ?>');">Export</a>
			            </li>
					</ul>
				</div>
			</div>
						...
						<div class="row">
							<div class="col-md-12">
								<ul class="nav nav-tabs selectedtab" id="mgaList">
									<li id="tabcharges" onclick="showcharges(); $('#printcharges').css('display','block');$('#printpayment').css('display', 'none'); " class="active"><a data-toggle="tab">Charges</a></li>
									<li id="tabpayment" onclick="showPayment(); $('#printpayment').css('display','block');$('#printcharges').css('display', 'none');"><a data-toggle="tab">Payment</a></li>
								</ul>

								<div class="parent reportlists" style="overflow-x: scroll;" id="showCharges">
									<table class="table table-bordered fixTable">
										<thead>
											...
											<th style="white-space:nowrap;">BILLING TYPE</th>
											<th style="white-space:nowrap;">CHARGES</th>
