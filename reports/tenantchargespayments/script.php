<script type="text/javascript">
	...
	function dbcharges(){
		...
		$.ajax ({
			type: 'POST',
			url: 'reports/tenantchargespayments/class.php',
			data: 'key=' + key + '&DateFrom=' + DateFrom + '&DateTo=' + DateTo + '&page=' + page + '&MallID=' + MallID +  '&form=dbcharges',
			success: function(data){
				// if(data != ""){
		          	$("#tblcharges").html(data);

		        // }
		        // else{
		        //   	$("#tblcharges").html("<tr><td colspan='10' style='text-align: center;'>No Data Found...</td></tr>");
		        // }
				fncTSREntries('dbcharges');
				fncTSRPage('dbcharges');
			}
		});
	}
	...
	function AutoConsolidatecharges(mallid){
		...
		$.ajax ({
			type: 'POST',
			url: 'reports/tenantchargespayments/class.php',
			data: 'tenant=' + tenant + '&datefrom=' + datefrom + '&dateto=' + dateto + '&mallid=' + mallid + '&form=AutoConsolidatecharges',
			success: function (data) {
				alert(data);
				if(data == 1){
					setTimeout(function(){
						showmodal("alert", "Tenant Charges successfully exported.", "", null, "", null, "0");
					}, 500)
				}else{
					setTimeout(function(){
						showmodal("alert", "Failed to export Tenant Charge.", "", null, "", null, "1");
					}, 500)
				}
			}
		})
	...
	function AutoConsolidatepayment(mallid){
		var tenant = $('#txtSearchTenantName').val();
		var datefrom = $('#dateFromList').val();
		var dateto = $('#dateToList').val();
		$.ajax ({
