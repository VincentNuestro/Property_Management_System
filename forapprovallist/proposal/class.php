<?php  
	...
	switch ($_POST['form']) {
		case 'tblListofApplication':
			...
			$StatFilter = "(a.Status = 'Pending' AND a.S2Leasing = '1')";
			...
			$page = $_POST["page"];
			$limit = ($page-1) * 20;
			$sql = "SELECT a.TradeID, a.Company_ID, a.Inquiry_ID, a.UnitID, a.Application_ID, a.Trade_Name, a.Company_Name, a.Industry, a.Company_ID, a.date_inquired, a.Status, a.req_status, a.UnitType, a.Mall_ID, a.month_adv, a.leadsID, a.S2Leasing, a.forFinal, a.inqSource, a.inqPrcssOwnr, b.proposalNum, b.stats, a.ActiveProposal FROM tbltrans_inquiry a, tbltrans_proposal b, tbluser c, tbltrans_hierarchy d WHERE a.Inquiry_ID = b.inquiryID AND b.userid = c.userid AND c.hierarchycode = d.hiecode AND  ". $StatFilter ." ". $SearchFilter ." ". $DateFilter ." ". getMallAccess("Mall_ID", "AND") ." AND a.isDirect = '0' AND a.isAmendment = '0' AND d.role = '".$chckrole['role']."' AND d.module = 'Proposal' AND d.ulevel = '".$chckrole['ulevel']."' ".$apps. " AND  b.stats = '0' ORDER BY a.Inquiry_ID DESC LIMIT ".$limit.", 20;";
			...
			while($row = mysql_fetch_array($result)){
                ...
                $UnitCount = 0;
                $UnitInfo = "";
                $resUnit = mysql_query("SELECT UnitID FROM tbltrans_proposal_unit WHERE InquiryID = '". $row['Inquiry_ID'] ."' AND proposalNum = '". $row['proposalNum'] ."';", $connection);
                ...
                $userapproved = mysql_fetch_array(mysql_query("SELECT CONCAT(lastname,', ',firstname) as fullname FROM tbluser WHERE userid = '".$row['1st_app']."' "));
				...
				echo "	<tr>
							...
							<td style='vertical-align: middle;'>".$userapproved[0]."</td>
							<td style='vertical-align: middle;'></td>
							<td style='vertical-align: middle;'><div class='btn-group'>";
										echo "<button class='btn btn-sm btn-purple  isadmin select-editapplication btn-round' onclick='fncEditGlobalFormInquiry(\"1\", \"".$row['Inquiry_ID']."\", \"".$row['Application_ID']."\", \"".$row['leadsID']."\", \"".$row['TradeID']."\", \"".$row['Company_ID']."\", \"".$row['proposalNum']."\", \"".$row['stats']."\");' title='Lease Proposal' style='margin: 2px;'><img src='assets/images/view.png' style='width: 100%; height: auto;' /></button>";
										echo "<button class='btn btn-sm btn-purple  isadmin select-editapplication btn-round' onclick='fncApproveProposal(\"".$row['Inquiry_ID']."\",\"".$row['proposalNum']."\")' title='Approve' style='margin: 2px;'><img src='assets/images/like.png' style='width: 100%; height: auto;' /></button>";
										echo "<button class='btn btn-sm btn-default isadmin select-editapplication btn-round'  title='Print' onclick='fncSelectReportType(\"".$row['Inquiry_ID']."\", \"Proposal\", \"". $row['proposalNum'] ."\");' style='margin: 2px;'><img src='assets/images/printer.png' style='width: 100%; height: auto;' /></button>";
									if($events == 1){
		                                echo"<button class='btn btn-sm btn-gray  isadmin select-viewevents btn-round' onclick='frmeventformop(\"". $row['Inquiry_ID'] ."\",".$events.",\"".$row['Company_ID']."\",1,\"".$row['ActiveProposal']."\",\"".$getProposalStatus['stats']."\")' title='View Event' style='margin: 2px;'><img src='assets/images/ticket.png' style='width: 100%; height: auto;' /></button>";
		                            }else{
		...
		case 'tblListofApplicationEntries':
		    ...
		    if($Date[0] != "" && $Date[1] != ""){
           	...
           	$limit = ($page-1) * 20;
  			$rowCount = mysql_fetch_array(mysql_query("SELECT COUNT(a.id) FROM tbltrans_inquiry a, tbltrans_proposal b, tbluser c, tbltrans_hierarchy d WHERE a.Inquiry_ID = b.inquiryID AND b.userid = c.userid AND c.hierarchycode = d.hiecode AND  ". $StatFilter ." ". $SearchFilter ." ". $DateFilter ." ". getMallAccess("Mall_ID", "AND") ." AND a.isDirect = '0' AND a.isAmendment = '0' AND d.role = '".$chckrole['role']."' AND d.module = 'Proposal' AND d.ulevel = '".$chckrole['ulevel']."' ".$apps. " AND  b.stats = '0';", $connection));
  			$rowsperpage = 20;
		...
		case 'tblListofApplicationPagination':
			...
			$page = $_POST["page"];
			$rowCount = mysql_fetch_row(mysql_query("SELECT COUNT(a.id) FROM tbltrans_inquiry a, tbltrans_proposal b, tbluser c, tbltrans_hierarchy d WHERE a.Inquiry_ID = b.inquiryID AND b.userid = c.userid AND c.hierarchycode = d.hiecode AND  ". $StatFilter ." ". $SearchFilter ." ". $DateFilter ." ". getMallAccess("Mall_ID", "AND") ." AND a.isDirect = '0' AND a.isAmendment = '0' AND d.role = '".$chckrole['role']."' AND d.module = 'Proposal' AND d.ulevel = '".$chckrole['ulevel']."' ".$apps. " AND  b.stats = '0';", $connection));
			$rowsperpage = 20;
			$range = 3;
			$totalpages = ceil($rowCount[0] / $rowsperpage);
			$prevpage;
		...
		case 'fncApproveProposalGo':
			...
			if($stat==1){
				$updates2 = " ,date_approved = NOW(), approved_by = '".getusername()."' ";
			}else{
				$updates2 = "";
			}
			...
			if($setups==""){
				...
				$updateinq = mysql_query("UPDATE tbltrans_inquiry SET alluserid = '".$getlevel['userid']."' WHERE Inquiry_ID = '".$_POST['InquiryID']."'; ",$connection);
				echo 1;
		...
		case 'saveapprovalproposalFilter':
			$chckrole = mysql_fetch_array(mysql_query("SELECT b.ulevel,b.role FROM tbluser a,tbltrans_hierarchy b WHERE a.groupaccess = b.role AND a.userid = '".$_SESSION['MMS-UserID']."' AND b.module = 'Proposal'   ",$connection));
			$filterstatusprop = "";
			$statusprop =  explode("|", $_POST['checked3']);
			if($statusprop[0]!=""){
				$filterstatusprop = "  AND b.stats = '".$statusprop[0]."' ";
				if($statusprop[1]!=""){
					$filterstatusprop = " AND (b.stats = '".$statusprop[0]."' OR b.stats = '".$statusprop[1]."') ";
				}
			}

			$filterdates = "";
			if($_POST['Date1']!="" && $_POST['Date2']!=""){
				$filterdates = " AND b.datecreated BETWEEN '".date('Y-m-d',strtotime($_POST['Date1']))."' AND  '".date('Y-m-d',strtotime($_POST['Date2']))."' ";
			}
			...
			$page = $_POST["page"];
			$limit = ($page-1) * 20;
			$sql = "SELECT a.TradeID, a.Company_ID, a.Inquiry_ID, a.UnitID, a.Application_ID, a.Trade_Name, a.Company_Name, a.Industry, a.Company_ID, a.date_inquired, a.Status, a.req_status, a.UnitType, a.Mall_ID, a.month_adv, a.leadsID, a.S2Leasing, a.forFinal, a.inqSource, a.inqPrcssOwnr, b.proposalNum, b.stats, b.1st_app, b.1st_date, a.S2Leasing, a.ActiveProposal FROM tbltrans_inquiry a, tbltrans_proposal b, tbluser c, tbltrans_hierarchy d WHERE a.Inquiry_ID = b.inquiryID AND b.userid = c.userid AND c.hierarchycode = d.hiecode ". getMallAccess("Mall_ID", "AND") ." AND d.role = '".$chckrole['role']."' AND d.module = 'Proposal' AND d.ulevel = '".$chckrole['ulevel']."' ".$filterstatusprop." ".$filterdates." ".$filtersearch."   ORDER BY a.Inquiry_ID DESC ;";	
			$result = mysql_query($sql, $connection);
			while($row = mysql_fetch_array($result)){
				if($row["stats"] == "0"){
					$stat = "<span class='label label-lg label-pink arrowed-in-right arrowed' style='z-index: 0;'>Pending</span>";
				}else{
					$stat = "<span class='label label-lg label-success arrowed-in-right arrowed' style='z-index: 0;'>Approved</span>";
				}
				...
				echo "	<tr>
							...
							<td style='vertical-align: middle;'><div class='btn-group'>";
										...
										echo "<button class='btn btn-sm btn-purple  isadmin select-editapplication btn-round' onclick='fncEditGlobalFormInquiry(\"1\", \"".$row['Inquiry_ID']."\", \"".$row['Application_ID']."\", \"".$row['leadsID']."\", \"".$row['TradeID']."\", \"".$row['Company_ID']."\", \"".$row['proposalNum']."\", \"".$row['stats']."\");' title='Lease Proposal' style='margin: 2px;'><img src='assets/images/view.png' style='width: 100%; height: auto;' /></button>";
										if($row['Status'] = 'Pending' && $row['S2Leasing'] = '1' && $row['stats']=='0'){
											echo "<button class='btn btn-sm btn-purple  isadmin select-editapplication btn-round' onclick='fncApproveProposal(\"".$row['Inquiry_ID']."\",\"".$row['proposalNum']."\")' title='Approve' style='margin: 2px;'><img src='assets/images/like.png' style='width: 100%; height: auto;' /></button>";	
										}
										if($events==1){
										...
										echo "<button class='btn btn-sm btn-default isadmin select-editapplication btn-round'  title='Print' onclick='fncSelectReportType(\"".$row['Inquiry_ID']."\", \"Proposal\", \"". $row['ActiveProposal'] ."\");' style='margin: 2px;'><img src='assets/images/printer.png' style='width: 100%; height: auto;' /></button>";
									echo"</div>
							</td>
						</tr>";
			}			
