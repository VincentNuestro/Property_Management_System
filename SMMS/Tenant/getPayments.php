...
$charges = mysql_query("SELECT SUM(amount) as totalpayments
FROM dunn_tblsoadetails WHERE amount < '0' AND paymenttype != '' AND xdate
 BETWEEN '$startdate' AND '$enddate' AND isMerchant = '0';
");
...
if (mysql_num_rows($charges) != 0) {
	...
	while($rowItem = mysql_fetch_array($charges)){
				...
				if ($rowItem["totalpayments"] == "null" || $rowItem["totalpayments"] == ""){$list["totalpayments"]= "";}else{$list["totalpayments"] = $rowItem["totalpayments"];}
				...
				if ($rowItem["xdescription"] == "null" || $rowItem["xdescription"] == ""){$list["xdescription"]= "";}else{$list["xdescription"] = $rowItem["qty"];}*/
