...
$selectAllPending = mysql_query("SELECT W.workorderid, W.tenantid, T.tradename, W.taskstatus,
...
INNER JOIN tbltrans_tenants T ON T.TenantID = W.tenantid
...
if(mysql_num_rows($selectAllPending)<>0){
	while($rowItem = mysql_fetch_array($selectAllPending)){
		...
		array_push($response["Tasklists"],$list);
	}	
	$response["success"] = 1;
}else{
	$response["success"] = 0;
