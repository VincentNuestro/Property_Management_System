...
if($validation == "1"){
	// $id = $_POST["id"];	
	$id = "USER-0000007";
		$SelectCounted = mysql_query("SELECT startdate, workorderid FROM `tblmaintenance_workorder` WHERE workerid = '$id'");
		if(mysql_num_rows($SelectCounted) <> 0){
			while($row = mysql_fetch_array($SelectCounted)){
				...
				WHERE W.`startdate` = '".$row["startdate"]."' AND C.`taskstatus` = 'Pending' AND W.`workerid` = '$id'"));
				$list["numberOfTask"] = $Pending["Counted"];
				
				$All = mysql_fetch_array(mysql_query("SELECT COUNT(C.id) AS Counted
				FROM `tblmaintenance_workorderlist` C
				INNER JOIN `tblmaintenance_workorder` W ON W.`workorderid`= C.`workorderid`
				WHERE W.`startdate` = '".$row["startdate"]."' AND C.`taskstatus` = 'Resolved' AND W.`workerid` = '$id'"));
